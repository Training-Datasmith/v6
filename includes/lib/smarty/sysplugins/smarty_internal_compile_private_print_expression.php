 ...
 * @subpackage Compiler
 */
class Smarty_Internal_Compile_Private_Print_Expression extends Smarty_internal_compile_Base
{
    /**
        ...
        if (isset($_attr['assign'])) {
            ...
            if (!$_attr['nofilter']) {
                ...
                if (!empty($compiler->smarty->default_modifiers)) {
                    ...
                    $output = $compiler->compile_tag('private_modifier', [], ['modifierlist' => $compiler->default_modifier_list, 'value' => $output]);
                }
                // autoescape html
                ...
                if (isset($compiler->smarty->autoload_filters[Smarty::FILTER_VARIABLE])) {
                    foreach ((array) $compiler->template->smarty->autoload_filters[Smarty::FILTER_VARIABLE] as $name) {
                        ...
                        if ($result !== false) {
                            ...
                            throw new Smarty_Exception("Unable to load variable filter '{$name}'");
                        }
                    }
                }
                foreach ($compiler->variable_filters as $filter) {
                    if (count($filter) === 1 && ($result = $this->compile_variable_filter($compiler, $filter[0], $output)) !== false) {
                        ...
                        $output = $compiler->compile_tag('private_modifier', [], ['modifierlist' => [$filter], 'value' => $output]);
                    }
                }
            }
     ...
     * @throws \SmartyException
     */
    private function compile_variable_filter(Smarty_internal_template_Compiler_Base $compiler, $name, $output)
    {
        $function = $compiler->get_plugin($name, 'variablefilter');
        if ($function) {
