...
class Smarty_Template_Config extends Smarty_Template_Source
{
    ...
    public static function load(Smarty_Internal_Template $_template = null, Smarty $smarty = null, $template_resource = null)
    {
        ...
        if (empty($template_resource)) {
            throw new Smarty_Exception('Source: Missing  name');
        }
        // parse resource_name, load resource handler
        list($name, $type) = Smarty_Resource::parse_resource_name($template_resource, $smarty->default_config_type);
        // make sure configs are not loaded via anything smarty can't handle
        if (isset($_incompatible_resources[$type])) {
            throw new Smarty_Exception("Unable to use resource '{$type}' for config");
        }
        ...
        if (!$source->exists && isset($smarty->default_config_handler_func)) {
            Smarty_internal_method_register_Default_Template_Handler::_get_default_template($source);
            $source->handler->populate($source, $_template);
        }
        return $source;
    }
