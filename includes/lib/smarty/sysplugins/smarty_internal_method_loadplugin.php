 ...
 * @author     Uwe Tews
 */
class Smarty_internal_method_load_Plugin
{
    /**
     ...
     */
    public function load_plugin(Smarty $smarty, $plugin_name, $check)
    {
        ...
        if (!preg_match('#^smarty_((internal)|([^_]+))_(.+)$#i', $plugin_name, $match)) {
            throw new Smarty_Exception("plugin {$plugin_name} is not a valid name format");
        }
        if (!empty($match[2])) {
        ...
        if (isset($this->plugin_files)) {
            ...
            if (!$smarty->use_include_path || $smarty->ext->_get_include_path->is_new_include_path($smarty)) {
                unset($this->plugin_files['include_path']);
            } else if (isset($this->plugin_files['include_path'][$_lower_filename])) {
        ...
        $_p_dirs = $smarty->get_plugins_dir();
        if (!isset($this->plugin_files['plugins_dir'][$_lower_filename])) {
        ...
        if ($smarty->use_include_path) {
            foreach ($_file_names as $_file_name) {
                ...
                $file = $smarty->ext->_get_include_path->get_include_path($_p_dirs, $_file_name, $smarty);
