 ...
 */
class Smarty extends Smarty_internal_template_Base
{
    /**
     * smarty version
     */
    public const SMARTY_VERSION = '4.3.0';
    /**
     ...
     * @var bool
     */
    public $_plugins_dir_normalized = false;
    /**
     * flag if template_dir is normalized
     *
     * @var bool
     ...
     * @var string[]
     */
    protected $obsolete_properties = ['resource_caching', 'template_resource_caching', 'direct_access_security', '_dir_perms', '_file_perms', 'plugin_search_order', 'inheritance_merge_compiled_includes', 'resource_cache_mode'];
    /**
     * List of private properties which will call getter/setter on a direct access
     *
     * @var string[]
     */
    protected $access_map = ['template_dir' => 'TemplateDir', 'config_dir' => 'ConfigDir', 'plugins_dir' => 'PluginsDir', 'compile_dir' => 'CompileDir', 'cache_dir' => 'CacheDir'];
    /**
     ...
     * @throws \SmartyException
     */
    public function enable_security($security_class = null)
    {
        Smarty_Security::enable_security($this, $security_class);
        return $this;
    }
    /**
     * Disable security
     ...
     * @return Smarty current Smarty instance for chaining
     */
    public function disable_security()
    {
     ...
     * @return Smarty          current Smarty instance for chaining
     */
    public function add_template_dir($template_dir, $key = null, $is_config = false)
    {
        if ($is_config) {
     ...
     * @return \Smarty current Smarty instance for chaining
     */
    public function set_template_dir($template_dir, $is_config = false)
    {
        if ($is_config) {
            $this->config_dir = [];
    ...
    public function get_plugins_dir()
    {
        ...
        if (!$this->_plugins_dir_normalized) {
            ...
            $this->_plugins_dir_normalized = true;
        }
    ...
    public function create_template($template, $cache_id = null, $compile_id = null, $parent = null, $do_clone = true)
    {
        ...
        $_template_id = $this->_get_template_id($template, $cache_id, $compile_id);
        $tpl = null;
        if ($this->caching && isset(Smarty_Internal_Template::$is_cache_tpl_obj[$_template_id])) {
            $tpl = $do_clone ? clone Smarty_Internal_Template::$is_cache_tpl_obj[$_template_id] : Smarty_Internal_Template::$is_cache_tpl_obj[$_template_id];
            $tpl->inheritance = null;
            $tpl->tpl_vars = $tpl->config_vars = [];
        } elseif (!$do_clone && isset(Smarty_Internal_Template::$tpl_obj_cache[$_template_id])) {
            $tpl = clone Smarty_Internal_Template::$tpl_obj_cache[$_template_id];
            $tpl->inheritance = null;
            $tpl->tpl_vars = $tpl->config_vars = [];
        } else {
        ...
        if ($this->debugging || $this->debugging_ctrl === 'URL') {
            ...
            if (!$this->debugging && $this->debugging_ctrl === 'URL') {
                $tpl->smarty->_debug->debug_url($tpl->smarty);
            }
        }
        return $tpl;
    }
     ...
     * @throws \SmartyException
     */
    public function load_plugin($plugin_name, $check = true)
    {
        return $this->ext->load_plugin->load_plugin($this, $plugin_name, $check);
    }
    /**
     ...
     * @throws \SmartyException
     */
    public function _get_template_id($template_name, $cache_id = null, $compile_id = null, $caching = null, Smarty_Internal_Template $template = null)
    {
        ...
        if (isset($template) && strpos($template_name, ':.') !== false || $this->allow_ambiguous_resources) {
            $_template_id = Smarty_Resource::get_unique_template_name(isset($template) ? $template : $this, $template_name) . "#{$cache_id}#{$compile_id}#{$caching}";
        } else {
            $_template_id = $this->_joined_template_dir . "#{$template_name}#{$cache_id}#{$compile_id}#{$caching}";
        }
        if (isset($_template_id[150])) {
            $_template_id = sha1($_template_id);
        }
        return $_template_id;
    }
    /**
     ...
     */
    public function _clear_template_cache()
    {
        Smarty_Internal_Template::$is_cache_tpl_obj = [];
        Smarty_Internal_Template::$tpl_obj_cache = [];
    }
    /**
     * @param boolean $use_sub_dirs
     */
    public function set_use_sub_dirs($use_sub_dirs)
    {
     ...
     */
    public function set_error_reporting($error_reporting)
    {
     ...
     */
    public function set_auto_literal($auto_literal = true)
    {
     ...
     */
    public function set_force_compile($force_compile)
    {
     ...
     */
    public function set_merge_compiled_includes($merge_compiled_includes)
    {
     ...
     */
    public function set_left_delimiter($left_delimiter)
    {
     ...
     * @param string
     */
    public function set_right_delimiter($right_delimiter)
    {
     ...
     */
    public function set_debugging($debugging)
    {
     ...
     */
    public function set_config_overwrite($config_overwrite)
    {
     ...
     */
    public function set_config_booleanize($config_booleanize)
    {
     ...
     */
    public function set_config_read_hidden($config_read_hidden)
    {
     ...
     */
    public function set_compile_locking($compile_locking)
    {
     ...
     */
    public function set_default_resource_type($default_resource_type)
    {
     ...
     */
    public function set_caching_type($caching_type)
    {
     ...
     * @param null $errors
     */
    public function test_install(&$errors = null)
    {
        Smarty_internal_test_Install::test_install($this, $errors);
    }
    /**
     * Get Smarty object
     *
    ...
    {
        if (isset($this->access_map[$name])) {
            $method = 'get' . $this->access_map[$name];
            return $this->{$method}();
        } elseif (isset($this->_cache[$name])) {
            return $this->_cache[$name];
        } elseif (in_array($name, $this->obsolete_properties)) {
            return null;
        } else {
    ...
    {
        if (isset($this->access_map[$name])) {
            $method = 'set' . $this->access_map[$name];
            $this->{$method}($value);
        } elseif (in_array($name, $this->obsolete_properties)) {
            return;
     ...
     */
    private function _normalize_dir($dir_name, $dir)
    {
     ...
     * @param bool $isConfig true for config_dir
     */
    private function _normalize_template_config($is_config)
    {
        if ($is_config) {
            $processed =& $this->_processed_config_dir;
            $dir =& $this->config_dir;
        ...
        $is_config ? $this->_config_dir_normalized = true : $this->_template_dir_normalized = true;
        $is_config ? $this->_joined_config_dir = join('#', $this->config_dir) : $this->_joined_template_dir = join('#', $this->template_dir);
    }
    /**
     ...
     */
    public function mute_expected_errors(): void
    {
        $this->is_muting_undefined_or_null_warnings = true;
    }
    /**
