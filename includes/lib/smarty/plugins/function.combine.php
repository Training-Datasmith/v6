...
function smarty_function_combine($params, &$smarty)
{
    ...
    foreach (['input', 'output'] as $key) {
        $params[$key] = str_replace(['{$SKIN_SUBSET}', '{$SKIN_FOLDER}'], [(string) $smarty->tpl_vars['SKIN_SUBSET']->value, (string) $smarty->tpl_vars['SKIN_FOLDER']->value], $params[$key]);
    }
    /**
     * Build combined file
     *
            ...
            if ($lastest_mtime > $last_cmtime) {
                ...
                if (flock($fh, LOCK_EX)) {
                    foreach ($filelist as $file) {
                        ...
                        if ($params['type'] == 'js') {
                            $min = Js_Min::minify($min) . ';';
                        } elseif ($params['type'] == 'css') {
                            $min = Css_Min::minify(preg_replace('/url\(((?>["\']?))(?!(\/|http(s)?:|data:|#))(.*?)\1\)/', 'url("' . dirname(CC_ROOT_REL . $file['name']) . '/$4")', $min));
                        } else {
    ...
    if (!isset($params['cache_file_name'])) {
        $params['cache_file_name'] = 'cache/' . short_hash($params['output']);
    }
