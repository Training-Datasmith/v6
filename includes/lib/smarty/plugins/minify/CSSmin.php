...
class Cs_Smin
{
    ...
    public function run($css = '', $linebreak_pos = false)
    {
        ...
        if ($l <= $css_chunk_length) {
            ...
            while ($i < $l) {
                $i += 50;
                // save iterations. 500 checks for a closing curly brace }
    ...
    private function min($css, $linebreak_pos)
    {
        ...
        if (preg_match_all('/clamp/i', $css, $matches, PREG_OFFSET_CAPTURE)) {
            ...
            foreach ($matches[0] as $match) {
                $clamp_start = $match[1] + $adj;
                $clamp_len = strpos($css, ';', $clamp_start) - $clamp_start + 1;
                $this->preserved_tokens[] = substr($css, $clamp_start, $clamp_len);
                $token_tring = self::TOKEN . (count($this->preserved_tokens) - 1) . '___';
                $css = substr_replace($css, $token_tring, $clamp_start, $clamp_len);
                $adj += strlen($token_tring) - $clamp_len;
            }
        }
    ...
    private function compress_hex_colors($css)
    {
        ...
        $short_safe = ['#808080' => 'gray', '#008000' => 'green', '#800000' => 'maroon', '#000080' => 'navy', '#808000' => 'olive', '#ffa500' => 'orange', '#800080' => 'purple', '#c0c0c0' => 'silver', '#008080' => 'teal', '#f00' => 'red'];
        while (preg_match($pattern, $css, $m, 0, $offset)) {
            $index = $this->index_of($css, $m[0], $offset);
            $last_index = $index + strlen($m[0]);
    ...
    private function normalize_int($size)
    {
        if (is_string($size)) {
            switch (substr($size, -1)) {
                case 'M':
                case 'm':
                    return (int) substr($size, 0, -1) * 1048576;
                case 'K':
                case 'k':
                    return (int) substr($size, 0, -1) * 1024;
                case 'G':
                case 'g':
                    return (int) substr($size, 0, -1) * 1073741824;
            }
        }
        return (int) $size;
    }
