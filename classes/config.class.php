...
class Config
{
    ...
    final protected function __construct(array $glob)
    {
        ...
        if (isset($GLOBALS['db'])) {
            $array_out = $this->_fetch_rows('config');
        }
     ...
     */
    public static function get_instance($glob = []): self
    {
    ...
    public function get($config_name, $element = '', $isset = false)
    {
        ...
        $this->_fetch_config($config_name);
        //Return it if found
     ...
     * @return bool
     */
    public function is_empty($config_name, $element)
    {
    ...
    public function set_session_config($config_name, $data): void
    {
     ...
     * @return mixed
     */
    private function _decode_value($raw)
    {
        if ($raw === null || $raw === '') {
            return $raw;
        }
     ...
     * @return array|false
     */
    private function _fetch_rows($name): false|array
    {
        if (!isset($GLOBALS['db'])) {
            return false;
        }
        ...
        foreach ($result as $row) {
            $array_out[$row['config_key']] = $this->_decode_value($row['config_value']);
        }
        return $array_out;
    }
    /**
     ...
     * @param string $name
     */
    private function _fetch_config($name): void
    {
        //Clean up the entire config array
        $this->_config[$name] = [];
        $array_out = $this->_fetch_rows($name);
        ...
        if (!empty($this->_config) && is_array($this->_config)) {
            ...
            foreach ($this->_config as $config => $data) {
                ...
                foreach ($data as $key => $value) {
                    $db->insert('CubeCart_config', ['name' => $config, 'config_key' => $key, 'config_value' => $this->_encode_value($value)]);
                }
            }
        }
    }
