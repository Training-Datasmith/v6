...
class GUI
{
    ...
    final protected function __construct($admin = false)
    {
        ...
        if (!$admin) {
            ...
            if ($GLOBALS['config']->get('config', 'skin_change')) {
                ...
                if (isset($_POST['select_skin']) && !empty($_POST['select_skin']) && ($switch = $_POST['select_skin']) || isset($_GET['select_skin']) && !empty($_GET['select_skin']) && $switch = $_GET['select_skin']) {
                    ...
                    httpredir(current_page(['select_skin']));
                }
            }
            ...
            if (!isset($_COOKIE['cc_browser'])) {
                $ccb_path = $GLOBALS['session']->get_cookie_path();
                $ccb_domain = $GLOBALS['session']->get_cookie_domain();
            ...
            if (($livehelp_plugins = $GLOBALS['db']->select('CubeCart_modules', ['folder'], ['module' => 'livehelp', 'status' => '1'])) !== false) {
                ...
                foreach ($livehelp_plugins as $plugin) {
                    ...
                    if (file_exists($file_path)) {
                        ...
                        $livehelp_html[] = $livehelp_plugin->invocation_html();
                    }
                }
            ...
            $skin_folder = !$GLOBALS['config']->is_empty('config', 'admin_skin') ? $GLOBALS['config']->get('config', 'admin_skin') : 'default';
            $admin_folder = !$GLOBALS['config']->is_empty('config', 'adminFolder') ? $GLOBALS['config']->get('config', 'adminFolder') : 'admin';
            $admin_file = !$GLOBALS['config']->is_empty('config', 'adminFile') ? $GLOBALS['config']->get('config', 'adminFile') : 'admin.php';
            $skin_root = $admin_folder . '/' . 'skins';
            if (strstr($skin_root, (string) CC_ROOT_DIR)) {
                $skindir = $skin_root;
            } else if (!$GLOBALS['config']->is_empty('config', 'admin_skin') && file_exists($skin_root . '/' . $skin_folder) && is_dir($skin_root . '/' . $skin_folder)) {
        ...
        define('SESSION_TOKEN', $GLOBALS['session']->get_token());
        ...
        $config['store_country_name'] = get_country_format($config['store_country']);
     ...
     * @param array $url
     */
    public function add_breadcrumb($name, $url = [], $replace = false): void
    {
        if ($replace) {
            $this->_breadcrumb = [];
        }
        if (is_array($url) && !empty($url)) {
            ...
            $url = ['_g' => $_GET['_g'], 'node' => !empty($this->_breadcrumb) && isset($_GET['node']) ? $_GET['node'] : 'index'];
            if (isset($_GET['mode'])) {
        ...
        if (!empty($name)) {
            $this->_breadcrumb[] = ['url' => $GLOBALS['seo']->se_oable($href), 'title' => ucfirst(strip_tags(str_replace('_', ' ', $name)))];
     ...
     */
    public function change_template_dir($directory = ''): void
    {
        if (empty($directory)) {
     ...
     * @param array $url
     */
    public function display_common(): void
    {
        if (!CC_IN_ADMIN) {
            if (!isset($_GET['_a']) || $_GET['_a'] != 'template') {
                $this->_display_language_switch();
                $this->_display_currency_switch();
                $this->_display_session_box();
                if (!isset($_GET['_a']) || !in_array($_GET['_a'], ['basket', 'cart', 'complete', 'checkout', 'confirm', 'gateway']) && !$GLOBALS['config']->get('config', 'catalogue_mode')) {
                    $this->display_side_basket();
                }
            }
            $this->_display_search_box();
            $this->_display_sale_items();
            $this->_display_mailing_list();
            $this->_display_documents();
            $this->_display_random_product();
            $this->_display_popular_products();
            $this->_display_skin_select();
            $this->_display_social();
            ...
            $GLOBALS['seo']->display_meta_data();
        }
        //Setup HTML lang
        $GLOBALS['smarty']->assign('HTML_LANG', $GLOBALS['language']->get_language());
        //Setup bread crumbs
        $GLOBALS['smarty']->assign('CRUMBS', $this->_breadcrumb);
        //Show any errors or warnings
        $this->_display_errors();
        // Display cookie warning is needed
        $this->_display_cookie_dialogue();
        // Display admin tools
        $this->_display_admin_tool_bar();
    }
    /**
     ...
     * @return string/bool
     */
    public function display_side_basket()
    {
        ...
        if (($contents = $GLOBALS['cart']->get()) !== false) {
            ...
            foreach ($contents as $hash => $product) {
                ...
                $product['total'] = $GLOBALS['tax']->price_format($product['price_display']);
                ...
                $product['image'] = $this->get_product_image($product['product_id']);
        ...
        $GLOBALS['smarty']->assign('CART_TOTAL', isset($this->_total) ? Tax::get_instance()->price_format($this->_total) : $GLOBALS['tax']->price_format($basket_total));
        $button = ['link' => $GLOBALS['storeURL'] . '/index.php?_a=basket', 'text' => $GLOBALS['language']->basket['view_basket']];
     ...
     */
    public function get_custom_module_skin(string $type = 'gateway', string $dirname = '', string $file_name = ''): string
    {
     ...
     * @return string
     */
    public function get_logo($absolute = false, $type = '')
    {
        if (!empty($type)) {
            $this->_set_logo($type);
     ...
     */
    public function get_product_image($product_id = false, $mode = 'small', $path_type = 'url')
    {
        ...
        if (is_numeric($product_id)) {
            ...
            if ($this->_product_images[$product_id]) {
                return $GLOBALS['catalogue']->image_path($this->_product_images[$product_id][0]['file_id'], $mode, $path_type);
            }
        }
        if (isset($this->_skin_data['images'][$mode])) {
            $default = $this->_skin_data['images'][$mode]['default'];
            if ($this->_style !== '' && $this->_skin_data['styles'][$this->_style]['images']) {
            ...
            if ($files) {
                ...
                return $GLOBALS['catalogue']->image_path(str_replace(['//', '/'], '/', $files[0]), $mode);
            }
        }
        return false;
    }
     ...
     * @return int
     */
    public function items_per_page($list_id = 'products', $page_key = 'perpage', $list_amount = 'default')
    {
    ...
    public function get_skin_config($path = '', ?string $foldername = ''): \Simple_Xml_Element|false
    {
        if (!empty($foldername)) {
        ...
        if (!empty($path) && file_exists($path)) {
            ...
            try {
                return new Simple_Xml_Element($xml);
            } catch (Exception $e) {
    ...
    public function list_skins()
    {
        ...
        foreach (glob(CC_ROOT_DIR . '/skins/*/config.xml') as $data_file) {
            $data = $this->get_skin_config($data_file);
            if ($data) {
                $skins[(string) $data->info->{'name'}]['info'] = ['name' => (string) $data->info->{'name'}, 'display' => (string) $data->info->{'display'}, 'version' => (string) $data->info->{'version'}, 'compatible' => ['min' => (string) $data->info->{'minVersion'}, 'max' => (string) $data->info->{'maxVersion'}], 'creator' => (string) $data->info->{'creator'}, 'homepage' => (string) $data->info->{'homepage'}, 'mobile' => (string) $data->info->{'mobile'} == 'true' ? true : false, 'responsive' => (string) $data->info->{'responsive'} == 'true' ? true : false, 'csrf' => (string) $data->info->{'csrf'} == 'true' ? true : false, 'newsletter_recaptcha' => (string) $data->info->{'newsletter_recaptcha'} == 'true' ? true : false, 'gravatar_ajax' => (string) $data->info->{'gravatar_ajax'} == 'true' ? true : false];
                ...
                if ($data->styles) {
                    ...
                    foreach ($data->styles->style as $style) {
                        ...
                        $record = ['name' => (string) $style->{'name'}, 'description' => (string) $style->{'description'}, 'directory' => $name, 'images' => $style->attributes()->{'images'} == 'true', 'default' => $style->attributes()->{'default'} == 'true'];
                ...
                if ($data->images) {
                    foreach ($data->images->image as $image) {
                        ...
                        $skins[(string) $data->info->{'name'}]['images'][(string) $attrib->{'reference'}] = ['maximum' => (int) $attrib->{'maximum'}, 'quality' => (int) $attrib->{'quality'}, 'default' => (string) $attrib->default];
                    }
                }
            }
     ...
     * @return int
     */
    public function per_page_splits($list_id = 'products', $page_key = 'perpage'): array|false
    {
        if (isset($this->_skin_data['layout'][$list_id]['perpage'])) {
            foreach ($this->_skin_data['layout'][$list_id]['perpage'] as $amount => $default) {
                ...
                $page_splits[] = ['selected' => $selected, 'url' => current_page(null, [$page_key => $amount]), 'amount' => $amount];
            }
            return $page_splits;
        }
        return false;
     ...
     */
    public function recaptcha_required(): bool
    {
     ...
     */
    public function recaptcha_validate(): void
    {
        if ($this->recaptcha_required()) {
            ...
            if ($GLOBALS['config']->get('config', 'recaptcha') == '5') {
                if (empty($_POST['cf-turnstile-response'])) {
                    ...
                    $data = ['secret' => $GLOBALS['config']->get('config', 'recaptcha_secret_key'), 'response' => $_POST['cf-turnstile-response']];
                ...
                if (empty($_POST['h-captcha-response'])) {
                    ...
                    $data = ['secret' => $GLOBALS['config']->get('config', 'recaptcha_secret_key'), 'response' => $_POST['g-recaptcha-response']];
                ...
                $data = ['secret' => $GLOBALS['config']->get('config', 'recaptcha_secret_key'), 'response' => $_POST['g-recaptcha-response'], 'remoteip' => get_ip_address()];
     ...
     * Rebuild logo
     */
    public function rebuild_logos(): void
    {
            ...
            if (is_array($data)) {
                ...
                foreach ($data as $style => $value) {
                    if (isset($custom['all'])) {
                        ...
                        $target = $this->_get_logo_default($skin, $style);
                    }
                ...
                if (isset($custom['all'])) {
                    ...
                    ## Look for default logo
                    $target = $this->_get_logo_default($skin);
                }
        ...
        foreach ($extra_templates as $type) {
            if (isset($custom[$type])) {
                ...
                $target = $this->get_logo();
            }
            $logo_config[$type] = $target;
        }
        $GLOBALS['config']->set('logos', false, str_replace('/', '/', $logo_config), true);
        // Save skin and extra templates
    }
    /**
     ...
     */
    public function set_error($message = null, $admin_only = false)
    {
     ...
     * @param bool admin_only
     */
    public function set_notify($message = null, $admin_only = false)
    {
        if ($admin_only && !ADMIN_CP) {
            return false;
        }
        $this->_error_message('notice', $message);
    }
    /**
     * Set a notification message
     *
     ...
     * @param bool admin_only
     */
    public function set_info($message = null, $admin_only = false)
    {
        if ($admin_only && !ADMIN_CP) {
            return false;
        }
        $this->_error_message('info', $message);
    }
    //=====[ Private ]=======================================
    /**
     ...
     * @return bool
     */
    private function _detect_mobile($useragent = '')
    {
    ...
    private function _display_admin_tool_bar()
    {
        ...
        } else if ($home = $GLOBALS['db']->select('CubeCart_documents', ['doc_id'], ['doc_home' => '1'])) {
     ...
     */
    private function _display_cookie_dialogue()
    {
        if (CC_IN_ADMIN) {
            return false;
        }
        ...
        if (Config::get_instance()->get('config', 'cookie_dialogue') && !isset($_COOKIE['cc_accept_cookies'])) {
            if ($privacy = $GLOBALS['db']->select('CubeCart_documents', 'doc_id', ['doc_privacy' => '1'])) {
                ...
                $dialogue = str_replace('%PRIVACY_URL%', $GLOBALS['seo']->build_url('doc', $privacy[0]['doc_id']), $dialogue);
            } else {
     ...
     */
    private function _display_currency_switch()
    {
        if (!$GLOBALS['smarty']->template_exists('templates/box.currency.php')) {
            return false;
        }
        if (($currencies = $GLOBALS['db']->select('CubeCart_currency', false, ['active' => '1'])) !== false) {
            if (count($currencies) > 0) {
                ...
                foreach ($currencies as $offset => $currency) {
                    ...
                    $currency['url'] = current_page(null, ['set_currency' => $currency['code']]);
     ...
     */
    private function _display_documents()
    {
        if (!$GLOBALS['smarty']->template_exists('templates/box.documents.php')) {
            return false;
        }
        $vars = [];
        if (($docs = $GLOBALS['db']->select('CubeCart_documents', false, ['doc_parent_id' => '0', 'doc_status' => '1', 'navigation_link' => 1], '`doc_order` ASC')) !== false) {
            foreach ($docs as $doc) {
                ...
                if ($GLOBALS['language']->translate_document($doc)) {
                    if (empty($doc['doc_url'])) {
                        $doc['doc_url'] = $GLOBALS['seo']->build_url('doc', $doc['doc_id']);
                    }
                    $vars[] = $doc;
                }
            }
        ...
        if (($contact = $GLOBALS['config']->get('Contact_Form')) !== false) {
            if (isset($contact['status']) && $contact['status']) {
                $contact_url = $GLOBALS['seo']->build_url('contact');
     ...
     * Display errors
     */
    private function _display_errors(): void
    {
     ...
     */
    private function _display_language_switch()
    {
        if (!$GLOBALS['smarty']->template_exists('templates/box.language.php')) {
            return false;
        }
        $lang_list = $GLOBALS['language']->list_languages();
            ...
            if (count($lang_list) > 1) {
                foreach ($lang_list as $language) {
                    ...
                    if (!empty($language['domain'])) {
                        ...
                        $language['url'] = current_page(null, ['set_language' => $language['code']]);
                    }
     ...
     */
    private function _display_mailing_list()
    {
        if (!$GLOBALS['smarty']->template_exists('templates/box.newsletter.php')) {
            return false;
        }
        if ($GLOBALS['user']->is()) {
        ...
        if (isset($_POST['subscribe'])) {
            ...
            if (isset($_POST['force_unsubscribe']) && $_POST['force_unsubscribe'] == '1') {
                $newsletter->unsubscribe($_POST['subscribe'], $GLOBALS['user']->get_id());
            } elseif ($newsletter->subscribe($_POST['subscribe'], $GLOBALS['user']->get_id())) {
                httpredir(current_page());
            } else {
                httpredir(current_page(null, ['_a' => 'newsletter']));
            }
        }
     ...
     */
    private function _display_navigation()
    {
        if (!$GLOBALS['smarty']->template_exists('templates/box.navigation.php')) {
            return false;
        }
        ...
        if (($content = $GLOBALS['cache']->read($cache_id, $serialize)) == false) {
            ...
            $tree_data = $GLOBALS['catalogue']->get_category_tree();
            //Make the navigation tree
            $navigation_tree = $this->_make_tree($tree_data);
            ...
            $url = ['saleitems' => $GLOBALS['seo']->build_url('saleitems', false, '&', false), 'certificates' => $GLOBALS['seo']->build_url('certificates', false, '&', false)];
     ...
     */
    private function _display_popular_products()
    {
        if (!$GLOBALS['smarty']->template_exists('templates/box.popular.php')) {
            return false;
        }
        ...
        if ($vars && is_array($vars)) {
            ...
            if ($products) {
                foreach ($products as $product) {
                    $category_data = $GLOBALS['catalogue']->get_category_status_by_product_id($product['product_id']);
     ...
     */
    private function _display_random_product()
    {
        if (!$GLOBALS['smarty']->template_exists('templates/box.featured.php')) {
            return false;
        }
        ...
        $featured_products = $GLOBALS['db']->select('CubeCart_inventory', ['product_id'], $GLOBALS['catalogue']->out_of_stock_where(['status' => '1', 'featured' => '1']), 'RAND()', 15, false, false);
        ...
        if ($n > 0) {
            ...
            while (!$random_product && $tries < 15) {
                ...
                $category_data = $GLOBALS['catalogue']->get_category_status_by_product_id($random_id);
                $category_status = false;
                if (is_array($category_data)) {
                    foreach ($category_data as $data) {
        ...
        if ($random_product) {
            $image = $this->get_product_image($random_product[0]['product_id']);
            ...
            if ($product['use_stock_level']) {
                // Get Stock Level
                $stock_level = $GLOBALS['catalogue']->get_product_stock($product['product_id']);
     ...
     */
    private function _display_sale_items(): bool
    {
        if (!$GLOBALS['smarty']->template_exists('templates/box.sale_items.php')) {
            return false;
        }
        ...
        // Hide out-of-stock
        $sale_sql_standard_where = $GLOBALS['catalogue']->out_of_stock_where($sale_sql_standard_where);
        $sale_sql_group_where = $GLOBALS['catalogue']->out_of_stock_where($sale_sql_group_where, 'I');
        // Check for group pricing
        if ($GLOBALS['user']->is() && ($memberships = $GLOBALS['user']->get_memberships()) !== false) {
            $group_id = [];
        ...
        foreach ($sorted_products as $product) {
            $GLOBALS['language']->translate_product($product);
            $product['name'] = valid_html($product['name']);
            $product['url'] = $GLOBALS['seo']->build_url('prod', $product['product_id']);
            $product['saving_unformatted'] = $product['saving'];
            $product['saving'] = $GLOBALS['tax']->price_format($product['saving']);
            $GLOBALS['catalogue']->get_product_price($product);
            $sale = $GLOBALS['tax']->sale_price($product['price'], $product['sale_price']);
            $product['price_unformatted'] = $product['price'];
            $product['sale_price_unformatted'] = $sale ? $product['sale_price'] : null;
        ...
        if (count($vars) > 0) {
            ...
            $GLOBALS['smarty']->assign('SALE_ITEMS_URL', $GLOBALS['seo']->build_url('saleitems'));
     ...
     * Display search box
     */
    private function _display_search_box()
    {
        if (!$GLOBALS['smarty']->template_exists('templates/box.search.php')) {
            return false;
        }
        foreach ($GLOBALS['hooks']->load('class.gui.display_search_box') as $hook) {
            include $hook;
        }
        $GLOBALS['smarty']->assign('SEARCH_URL', $GLOBALS['seo']->build_url('search'));
        $GLOBALS['smarty']->assign('SEARCH_FORM', $GLOBALS['smarty']->fetch('templates/box.search.php'));
    }
    /**
     * Display session box
     */
    private function _display_session_box()
    {
        if (!$GLOBALS['smarty']->template_exists('templates/box.session.php')) {
            return false;
        }
        if ($GLOBALS['user']->is()) {
        ...
        $GLOBALS['smarty']->assign('URL', ['login' => $GLOBALS['seo']->build_url('login'), 'register' => $GLOBALS['seo']->build_url('register')]);
     ...
     */
    private function _display_skin_select()
    {
        if (!$GLOBALS['smarty']->template_exists('templates/box.skins.php')) {
            return false;
        }
        $skin_setting = $GLOBALS['config']->get('config', 'skin_change');
        if ($skin_setting == '1' || $skin_setting == '2' && Admin::get_instance()->is()) {
     ...
     */
    private function _display_social()
    {
        if (!$GLOBALS['smarty']->template_exists('templates/element.social.php')) {
            return false;
        }
        $vars = [];
        $config = $GLOBALS['config']->get('config');
        if (!empty($config['facebook'])) {
            $vars[] = ['url' => filter_var($config['facebook'], FILTER_VALIDATE_URL) ? $config['facebook'] : 'https://www.facebook.com/' . $config['facebook'], 'name' => 'Facebook', 'icon' => 'facebook-square'];
        }
        if (!empty($config['flickr'])) {
            $vars[] = ['url' => filter_var($config['flickr'], FILTER_VALIDATE_URL) ? $config['flickr'] : 'https://www.flickr.com/photos/' . $config['flickr'], 'name' => 'Flickr', 'icon' => 'flickr'];
        }
        if (!empty($config['bsky'])) {
            $vars[] = ['url' => filter_var($config['bsky'], FILTER_VALIDATE_URL) ? $config['bsky'] : 'https://bsky.app/profile/' . $config['bsky'], 'name' => 'Bluesky', 'icon' => 'bsky'];
        }
        if (!empty($config['instagram'])) {
            $vars[] = ['url' => filter_var($config['instagram'], FILTER_VALIDATE_URL) ? $config['instagram'] : 'https://www.instagram.com/' . $config['instagram'], 'name' => 'Instagram', 'icon' => 'instagram'];
        }
        if (!empty($config['linkedin'])) {
            $vars[] = ['url' => filter_var($config['linkedin'], FILTER_VALIDATE_URL) ? $config['linkedin'] : 'https://www.linkedin.com/company/' . $config['linkedin'], 'name' => 'Linkedin', 'icon' => 'linkedin-square'];
        }
        if (!empty($config['pinterest'])) {
            $vars[] = ['url' => filter_var($config['pinterest'], FILTER_VALIDATE_URL) ? $config['pinterest'] : 'https://www.pinterest.com/' . $config['pinterest'], 'name' => 'Pinterest', 'icon' => 'pinterest-square'];
        }
        if (!empty($config['twitter'])) {
            $vars[] = ['url' => filter_var($config['twitter'], FILTER_VALIDATE_URL) ? $config['twitter'] : 'https://x.com/' . $config['twitter'], 'name' => 'Twitter', 'icon' => 'twitter-square'];
        }
        if (!empty($config['vimeo'])) {
            $vars[] = ['url' => filter_var($config['vimeo'], FILTER_VALIDATE_URL) ? $config['vimeo'] : 'https://vimeo.com/' . $config['vimeo'], 'name' => 'Vimeo', 'icon' => 'vimeo-square'];
        }
        if (!empty($config['wordpress'])) {
            $vars[] = ['url' => filter_var($config['wordpress'], FILTER_VALIDATE_URL) ? $config['wordpress'] : 'https://' . $config['wordpress'] . '.wordpress.com', 'name' => 'WordPress', 'icon' => 'wordpress'];
        }
        if (!empty($config['youtube'])) {
            $vars[] = ['url' => filter_var($config['youtube'], FILTER_VALIDATE_URL) ? $config['youtube'] : 'https://www.youtube.com/user/' . $config['youtube'], 'name' => 'YouTube', 'icon' => 'youtube-square'];
        }
        if (!empty($config['reddit'])) {
            $vars[] = ['url' => filter_var($config['reddit'], FILTER_VALIDATE_URL) ? $config['reddit'] : 'https://www.reddit.com/r/' . $config['reddit'], 'name' => 'Reddit', 'icon' => 'reddit-square'];
        }
        if (!empty($config['tumblr'])) {
            $vars[] = ['url' => filter_var($config['tumblr'], FILTER_VALIDATE_URL) ? $config['tumblr'] : 'https://www.tumblr.com/' . $config['tumblr'], 'name' => 'Tumblr', 'icon' => 'tumblr-square'];
        }
     ...
     * @param string $message
     */
    private function _error_message(string $type, $message): void
    {
        if (!empty($message)) {
            if (is_array($message)) {
     ...
     */
    private function _get_logo_default(string $skin = '', string $style = ''): ?string
    {
        if (empty($skin)) {
     ...
     * @return string
     */
    private function _make_tree($tree_data): false|string
    {
        if (!$GLOBALS['smarty']->template_exists('templates/element.navigation_tree.php')) {
            return false;
        }
        $out = '';
        if (is_array($tree_data)) {
            foreach ($tree_data as $branch) {
                if (isset($branch['children'])) {
                    $branch['children'] = $this->_make_tree($branch['children']);
                }
                $branch['url'] = $GLOBALS['seo']->build_url('cat', $branch['cat_id'], '&', false);
     ...
     * Set canonical
     */
    private function _set_canonical(): bool
    {
        $canonical = $GLOBALS['smarty']->get_template_vars('CANONICAL');
        if (empty($canonical) && !is_numeric($canonical)) {
            ...
            $GLOBALS['smarty']->assign('CANONICAL', current_page($excluded, null, false));
            return true;
        }
        return false;
    }
     ...
     * @param string $type
     */
    private function _set_logo($type = ''): void
    {
        if (!empty($type)) {
     ...
     */
    private function _set_skin(): void
    {
        ...
        } else if (is_array($this->_skins) && !empty($this->_skins)) {
        ...
        if (($custom = $GLOBALS['cache']->read('skin.' . $this->_skin . '.custom')) === false && file_exists(CC_ROOT_DIR . '/skins/' . $this->_skin . '/' . 'config.xml')) {
            $xml = $this->get_skin_config('', $this->_skin);
            $custom = [];
            if (isset($xml->custom)) {
                foreach ($xml->custom->children() as $element) {
                    $custom[$element->get_name()] = (string) $element;
                }
            }
     ...
     */
    private function _set_style(): void
    {
