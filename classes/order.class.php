...
class Order
{
     ...
     * @return bool
     */
    public function add_note($order_id = null, $note = null, $print = true)
    {
        if (!empty($order_id) && !empty($note)) {
            $record = ['cart_order_id' => $order_id, 'time' => time(), 'content' => $note, 'print' => $print ? '1' : '0'];
     ...
     * @param any $admin
     */
    public function assign_order_details($values = null, $admin = null): void
    {
        ...
        if (!empty($this->_email_details['order_summary']['ship_tracking'])) {
            $this->_email_details['order_summary']['ship_tracking'] = nl2br(parse_url_to_link($this->_email_details['order_summary']['ship_tracking']));
        }
     ...
     * @return bool
     */
    public function create_download($product_id, $order_inv_id, $customer_id = '', $order_id = '')
    {
        if (!empty($order_id)) {
        ...
        return $this->_create_download($product_id, $order_inv_id);
    }
    /**
     ...
     * @return string/true
     */
    public function create_order_id($return = false, $set_basket = true)
    {
        ...
        if ($set_basket) {
            ...
            if (method_exists(Cart::get_instance(), 'save')) {
                Cart::get_instance()->save();
            }
        }
     ...
     */
    public function delete_card($cart_order_id): bool
    {
    ...
    public function delete_order($order_id)
    {
        if ((int) $GLOBALS['config']->get('config', 'stock_change_time') == 2) {
            $this->_manage_stock(self::ORDER_CANCELLED, $order_id);
        }
     ...
     * @return bool
     */
    public function pin_order($order_id, $pin = 1)
    {
     ...
     */
    public function disable_admin_email(): void
    {
    ...
    {
        $order_summary = $this->get_summary($order_id);
        $this->_get_inventory($order_id);
        ...
        $order_summary['subtotal'] = Tax::get_instance()->price_format($order_summary['subtotal'], true);
        $order_summary['show_credit'] = $order_summary['credit_used'] > 0 ? true : false;
        $order_summary['credit_used'] = Tax::get_instance()->price_format($order_summary['credit_used'], true);
        $order_summary['total'] = Tax::get_instance()->price_format($order_summary['total'], true);
        $order_summary['discount'] = Tax::get_instance()->price_format($order_summary['discount'], true);
        $order_summary['shipping'] = Tax::get_instance()->price_format($order_summary['shipping'], true);
        // Get taxes
        ...
        foreach ($this->_order_inventory as $item) {
            if ($item['product_id'] > 0) {
                $existing_data = $GLOBALS['catalogue']->get_product_data($item['product_id']);
                ...
                $product['item_price'] = Tax::get_instance()->price_format($product['price']);
                $product['price'] = Tax::get_instance()->price_format($product['price'] * $product['quantity']);
                $images = [];
                    ...
                    foreach ($gallery as $image) {
                        if (!in_array($image['file_id'], $duplicates)) {
                            ...
                            foreach ($image_types as $type) {
                                $image[$type] = $GLOBALS['catalogue']->image_path($image['file_id'], $type, 'url');
                            }
                            $images[] = $image;
                        }
                    }
                ...
                $options = $this->un_serialize_options($item['product_options']);
                $product['product_options'] = implode(' ', $options);
                $vars['products'][] = $product;
            } else {
                $item['price'] = Tax::get_instance()->price_format($item['price']);
                $vars['products'][] = $item;
            }
        }
        // Put tax in
        if ($order_taxes) {
            foreach ($order_taxes as $order_tax) {
                $tax_data = Tax::get_instance()->fetch_tax_details($order_tax['tax_id']);
                $tax['tax_name'] = $tax_data['name'];
                //$tax['tax_percent'] = sprintf('%.3F',$tax_data['tax_percent']);
                $tax['tax_percent'] = floatval($tax_data['tax_percent']);
                // get rid of zeroes
                $tax['tax_amount'] = Tax::get_instance()->price_format($order_tax['amount']);
                $vars['taxes'][] = $tax;
            }
        }
        $billing = ['first_name' => $order_summary['first_name'], 'last_name' => $order_summary['last_name'], 'company_name' => $order_summary['company_name'], 'line1' => $order_summary['line1'], 'line2' => $order_summary['line2'], 'town' => $order_summary['town'], 'state' => get_state_format($order_summary['state']), 'postcode' => $order_summary['postcode'], 'country' => get_country_format($order_summary['country']), 'phone' => $order_summary['phone'], 'email' => $order_summary['email'], 'w3w' => $order_summary['w3w']];
        $shipping = ['first_name' => $order_summary['first_name_d'], 'last_name' => $order_summary['last_name_d'], 'company_name' => $order_summary['company_name_d'], 'line1' => $order_summary['line1_d'], 'line2' => $order_summary['line2_d'], 'town' => $order_summary['town_d'], 'state' => get_state_format($order_summary['state_d']), 'postcode' => $order_summary['postcode_d'], 'country' => get_country_format($order_summary['country_d']), 'w3w' => $order_summary['w3w_d']];
        // Format data
        $order_summary['order_date'] = format_time($order_summary['order_date'], false, true);
        $order_summary['ship_date'] = (int) str_replace('-', '', (string) $order_summary['ship_date']) > 0 ? format_dispatch_date($order_summary['ship_date']) : '';
     ...
     * @return array/false
     */
    public function get_summary($order_id = null)
    {
     ...
     */
    public function log_transaction($log, $force_log = false): bool
    {
        // Log the transaction data returned from the payment gateways
        if (is_array($log) && !empty($log)) {
            $log['notes'] ??= '';
            $record = ['time' => time(), 'order_id' => $log['order_id'] ?? $this->_order_id, 'gateway' => $log['gateway'] ?? '', 'trans_id' => $log['trans_id'] ?? '', 'amount' => $log['amount'] ?? $this->_basket['total'], 'status' => $log['status'] ?? '', 'customer_id' => $log['customer_id'] ?? '', 'extra' => $log['extra'] ?? '', 'notes' => is_array($log['notes']) ? implode('<br>', $log['notes']) : $log['notes']];
     ...
     * @param bool $force
     */
    public function order_status($status_id, ?string $order_id, $force = false, $send_email = true): bool
    {
        ...
        if (!empty($status_id) && !empty($order_id)) {
            $current_status = $GLOBALS['db']->select('CubeCart_order_summary', ['status'], ['cart_order_id' => $order_id], false, false, false, false);
            if (!$current_status || (int) $current_status[0]['status'] == 0) {
                return false;
            }
            // no order record
            // Insert order status if it's changed
            if ((int) $status_id !== (int) $current_status[0]['status'] || $force) {
                $this->_add_history($order_id, $status_id);
                $this->_email_enabled = true;
            } else {
                // Don't send out emails already sent!
            ...
            switch ($status_id) {
                case self::ORDER_PENDING:
                    ...
                    if ($GLOBALS['config']->get('config', 'admin_notify_status') == '1' && $this->_email_admin_enabled && $admin_notify = $this->_notify_admins()) {
                        $admin_mailer = new Mailer();
                        $message_id = md5('admin.order_received' . $status_id . $order_id);
                        if (!$GLOBALS['session']->has($message_id, 'email') && ($content = $admin_mailer->load_content('admin.order_received')) !== false) {
                            $this->assign_order_details(null, true);
                            foreach ($GLOBALS['hooks']->load('class.order.order_status.admin_notify') as $hook) {
                                include $hook;
                            }
                            $admin_mailer->send_email($admin_notify, $content);
                ...
                case self::ORDER_PROCESS:
                    ...
                    if (!empty($order_summary['gateway']) && file_exists(CC_ROOT_DIR . '/modules/gateway/' . $order_summary['gateway'] . '/gateway.class.php')) {
                        ...
                        if (method_exists($gateway, 'processingEmail')) {
                            if (!isset($GLOBALS['cart'])) {
                                $GLOBALS['cart'] = (object) ['basket' => []];
                                // phantom object to prevent PHP 8 getting upset about calls in the gateway construct to $GLOBALS['cart']
                                $cart_is_phantom = true;
                            }
                            $already_sent = $gateway->processing_email($order_summary['cart_order_id']);
                        }
                    }
                    if ($cart_is_phantom) {
                        unset($GLOBALS['cart']);
                    ...
                    if (!$already_sent && $this->_email_enabled && ($content = $mailer->load_content('cart.order_confirmation', $order_summary['lang'])) !== false) {
                        $this->assign_order_details();
                        $mailer->send_email($this->_order_summary['email'], $content);
                    }
                    unset($content);
                    // Send email to store admins if set for processing status
                    if ($GLOBALS['config']->get('config', 'admin_notify_status') == '2' && $this->_email_enabled && $this->_email_admin_enabled && $admin_notify = $this->_notify_admins()) {
                        $admin_mailer = new Mailer();
                        $message_id = md5('admin.order_received' . $status_id . $order_id);
                        if (!$GLOBALS['session']->has($message_id, 'email') && ($content = $admin_mailer->load_content('admin.order_received')) !== false) {
                            $this->assign_order_details(null, true);
                    ...
                    if ($this->_email_enabled) {
                        foreach ($this->_order_inventory as $item) {
                            ...
                            if (!empty($item['custom']) && !empty($item['coupon_id']) && $item['digital']) {
                                $this->_send_coupon($item['coupon_id'], unserialize($item['custom']));
                            }
                        }
                    }
                    // Send digital files
                    $this->_digital_delivery($order_id, $this->_order_summary['email']);
                    ...
                    if ($order_summary['credit_used'] > 0 && $order_summary['credit_shift'] == 0) {
                        ...
                        if ($invalid_orders = $GLOBALS['db']->select('CubeCart_order_summary', ['cart_order_id'], ['status' => 1, 'credit_shift' => 0, 'credit_used' => '>0', 'customer_id' => $order_summary['customer_id']])) {
                            foreach ($invalid_orders as $o) {
                                $this->order_status(6, $o['cart_order_id']);
                                $this->add_note($o['cart_order_id'], sprintf($GLOBALS['language']->orders['cancelled_invalid_credit'], $order_summary['cart_order_id']));
                            }
                        }
                    }
                    if (isset($order_summary['coupon_data']) && !empty($order_summary['coupon_data'])) {
                        ...
                        $this->_process_coupons($order_summary['customer_id'], $order_summary['email'], $coupon_data);
                    }
                    break;
                case self::ORDER_COMPLETE:
                    ...
                    if ($GLOBALS['db']->select('CubeCart_order_history', ['status'], ['cart_order_id' => $order_id, 'status' => 2], false, false, false, false) === false) {
                        ...
                        if (!$GLOBALS['config']->get('config', 'no_skip_processing_check')) {
                            $this->order_status(2, $order_id, $force, $send_email);
                        } else {
                    ...
                    if (!$this->_skip_order_complete_email && $this->_email_enabled && ($content = $mailer->load_content('cart.order_complete', $order_summary['lang'])) !== false) {
                        $this->assign_order_details();
                        $mailer->send_email($this->_order_summary['email'], $content);
                    }
                    unset($content);
                ...
                case self::ORDER_FAILED:
                    ...
                    $content = $mailer->load_content('cart.payment_fraud', $order_summary['lang'], $this->_order_summary);
                    break;
                case self::ORDER_CANCELLED:
                    // Cancelled
                    $content = $mailer->load_content('cart.order_cancelled', $order_summary['lang'], $this->_order_summary);
                    break;
                default:
            ...
            $this->update_summary($order_id, $data);
            // Update Stock Levels
            $this->_manage_stock($status_id, $order_id);
     ...
     * @param string $order_id
     */
    public function payment_status($status_id, $order_id)
    {
        if (!empty($status_id) && !empty($order_id)) {
            $this->get_summary($order_id);
            ...
            switch ($status_id) {
                ...
                case self::PAYMENT_SUCCESS:
                    $content = $mailer->load_content('cart.payment_received', $this->_order_summary['lang'], $this->_order_summary);
                    break;
            }
            if ($this->_email_enabled && isset($content)) {
                $mailer->send_email($this->_order_summary['email'], $content);
     ...
     * @param bool $force
     */
    public function place_order($force_order = false): bool
    {
        ...
        if ($_GET['retrieve'] && isset($_GET['cart_order_id']) && !empty($_GET['cart_order_id'])) {
            // Order retrieval
            if ($this->_retrieve_order($_GET['cart_order_id'])) {
                httpredir(current_page(['cart_order_id', 'retrieve'], ['_a' => 'confirm']));
            }
            ...
            if (!isset($this->_basket['contents'])) {
                $GLOBALS['gui']->set_error($GLOBALS['language']->orders['expired_basket'], true);
                httpredir('index.php');
                return false;
            }
            ...
            if (isset($this->_basket['cart_order_id']) && !empty($this->_basket['cart_order_id']) && $GLOBALS['db']->select('CubeCart_order_summary', ['id'], ['cart_order_id' => $this->_basket['cart_order_id'], 'status' => 1], false, false, false, false) && !$GLOBALS['db']->select('CubeCart_transactions', ['id'], ['order_id' => $this->_basket['cart_order_id']], false, false, false, false)) {
                ...
                foreach ($this->_basket['contents'] as $key => $item) {
                    $product = $this->_order_add_product($item, $key);
            ...
            if (isset($this->_basket['shipping']) && is_array($this->_basket['shipping']) && !empty($this->_basket['shipping'])) {
                Tax::get_instance()->product_tax($this->_basket['shipping']['value'], (int) $this->_basket['shipping']['tax_id'], false, 0, 'shipping');
            }
            // Insert Taxes
            ...
            if (isset($this->_basket['coupons']) && is_array($this->_basket['coupons'])) {
                ...
                $this->add_note($this->_order_id, $note_content);
            }
            ...
            if (!empty($module_folder)) {
                ...
                if ($packing_note = $GLOBALS['session']->get($packing_session_key)) {
                    $this->add_note($this->_order_id, $packing_note, false);
                }
            }
            ...
            $this->_order_summary($update, $force_order);
            foreach ($GLOBALS['hooks']->load('class.order.place_order.postbasket') as $hook) {
                include $hook;
            }
            $this->_manage_stock(self::ORDER_PENDING, $this->_basket['cart_order_id']);
            $this->order_status(self::ORDER_PENDING, $this->_basket['cart_order_id'], true);
            if ($this->_basket['total'] == 0) {
                $this->order_status(self::ORDER_PROCESS, $this->_basket['cart_order_id']);
                httpredir(current_page(null, ['_a' => 'complete']));
            }
            return true;
        }
        // Go back to the basket page
        httpredir(current_page(['cart_order_id'], ['_a' => 'basket']));
        return false;
    }
    private function _process_coupons($customer_id, $email, $coupon_data = []): void
    {
     ...
     * @return array
     */
    public function un_serialize_options($option_string)
    {
     ...
     * @param int $product_id
     */
    public function serialize_options($options, $product_id): string
    {
        if (isset($options) && !empty($options)) {
            foreach ($options as $option_id => $assign_id) {
                if (!is_array($assign_id)) {
                    if (($value = $GLOBALS['catalogue']->get_option_data((int) $option_id, (int) $assign_id)) !== false) {
                        foreach ($GLOBALS['hooks']->load('class.cart.get.product_option_prices') as $hook) {
                            include $hook;
                        }
                        $value['price_display'] = '';
                        if (isset($value['option_price']) && $value['option_price'] > 0) {
                            // record option price but not zero
                            ...
                            $value['price_display'] .= Tax::get_instance()->price_format($value['option_price'], true, true) . ')';
                        }
                        $option[$assign_id] = $value['option_name'] . ': ' . $value['value_name'] . $value['price_display'];
                    }
                } else {
                    foreach ($assign_id as $id => $option_value) {
                        $textfield = $GLOBALS['db']->select('CubeCart_option_group', ['option_name', 'option_type'], ['option_id' => $option_id]);
                        // Kill me
                        if ($textfield && in_array($textfield[0]['option_type'], [1, 2])) {
                            ...
                            if (($value = $GLOBALS['catalogue']->get_option_data((int) $option_id, $assign_id)) !== false) {
                                foreach ($GLOBALS['hooks']->load('class.cart.get.product_option_prices') as $hook) {
                                    include $hook;
                                }
                                $value['price_display'] = '';
     ...
     * @return boolean
     */
    public function set_order_custom_id($cart_order_id, $column = 'cart_order_id')
    {
     ...
     * @return array/string
     */
    public function set_order_format($oid_prefix, $oid_postfix, $oid_zeros, $oid_start, $set = false, $force_past_oids = false, $oid = 1)
    {
        ...
        $concat_params = $this->_format_concat($oid_prefix) . ", {$lpad}, " . $this->_format_concat($oid_postfix);
        ...
        if ($set) {
            ...
            return ['oid_prefix' => $oid_prefix, 'oid_postfix' => $oid_postfix, 'oid_zeros' => $oid_zeros, 'oid_start' => $oid_start, 'oid_col' => $oid_col];
        }
        if ($oid > 0) {
     ...
     * @return bool
     */
    public function store_trans($trans_data, $force_log = false)
    {
        return $this->log_transaction($trans_data, $force_log);
    }
    /**
     ...
     */
    public function update_summary($order_id, $data_array): bool
    {
        ## Add notes, update status, gateway, shipping date, courier tracking url
        if (!empty($data_array) && is_array($data_array)) {
            if (!in_array($data_array['status'], ['1', '2'])) {
                $data_array['offline_capture'] = '';
            }
            // GitHub #1886
            $GLOBALS['db']->update('CubeCart_order_summary', $data_array, ['cart_order_id' => $order_id]);
            return true;
        }
        return false;
    }
     ...
     * @param string $order_id
     */
    public static function valid_order_id($order_id, $traditional = false): bool
    {
     ...
     * @return bool
     */
    private function _add_history($order_id, $status_id, string $initiator = '')
    {
        ...
        if (!empty($order_id) && !empty($status_id)) {
            $record = ['cart_order_id' => $order_id, 'updated' => time(), 'status' => $status_id, 'initiator' => $initiator];
     ...
     * @param string $glue
     */
    private function _create_certificate($value, $blocks = 5, $bsize = 4, $glue = '-'): int
    {
        ...
        $record = ['cart_order_id' => $this->_order_id, 'discount_price' => $value, 'code' => strtoupper($code)];
     ...
     * @return bool
     */
    private function _create_download($product_id, $order_inv_id)
    {
        ...
        $record = ['cart_order_id' => $this->_order_summary['cart_order_id'] ?? $this->_order_id, 'order_inv_id' => $order_inv_id, 'customer_id' => $customer_id, 'product_id' => (int) $product_id, 'expire' => $expire, 'accesskey' => $accesskey];
     ...
     * @return bool
     */
    private function _digital_delivery(string $order_id, $email)
    {
        if (!empty($order_id) && !empty($email)) {
            if (($digital = $GLOBALS['db']->select('CubeCart_downloads', false, ['cart_order_id' => $order_id], false, false, false, false)) !== false) {
                foreach ($digital as $download) {
                    ...
                    if (!$GLOBALS['config']->is_empty('config', 'download_expire')) {
                    ...
                    $data = $filemanager->get_file_info($download['product_id']);
                    $dkeys[] = ['stream' => $data['stream'], 'accesskey' => $download['accesskey'], 'name' => $product[0]['name'], 'expire' => $expire > 0 ? format_time($expire, false, true) : $GLOBALS['language']->common['never']];
                }
                $mailer = new Mailer();
                if ($this->_email_enabled && $contents = $mailer->load_content('cart.digital_download', $this->_order_summary['lang'], $this->_order_summary)) {
                    ...
                    return $mailer->send_email($email, $contents);
                }
            }
        }
        return false;
     ...
     * @param string $string
     */
    private function _format_concat(string|array|null $string): string
    {
        if (strstr($string, '%')) {
     ...
     * @return bool
     */
    private function _get_inventory($order_id = null)
    {
        if (!is_null($order_id)) {
     ...
     * @param string $order_id
     */
    private function _manage_stock($status_id, $order_id): bool
    {
     ...
     * @return string
     */
    private function _notify_admins()
    {
     ...
     * @return array/false
     */
    private function _order_add_product($item, $hash = '')
    {
        ...
        if (is_array($item)) {
            if (isset($item['certificate'])) {
                ...
                $product = [
                    ...
                    'tax_type' => 0,
                    // Gift certificates are MPV - no VAT at point of sale
                    'coupon_id' => $this->_create_certificate($item['certificate']['value']),
                    'custom' => serialize($item['certificate']),
                    'hash' => $hash,
                ];
            } else {
                $product = $GLOBALS['catalogue']->get_product_data($item['id'], 1, false, 10, 1, false, $item['options_identifier']);
            }
            $record = ['cart_order_id' => $this->_order_id, 'product_id' => (int) $item['id'], 'quantity' => $item['quantity'], 'cost_price' => number_format((float) $item['cost_price'], 2, '.', ''), 'price' => !isset($item['certificate']) ? $item['total_price_each'] : $item['certificate']['value'], 'tax' => $item['tax_each']['amount'], 'tax_percent' => $item['tax_each']['tax_percent'], 'product_code' => !empty($product['product_code']) ? $product['product_code'] : $item['product_code'], 'name' => !empty($product['name']) ? $product['name'] : $item['name'], 'digital' => !empty($product['digital']) || !empty($product['digital_path']) ? 1 : 0, 'custom' => $product['custom'] ?? null, 'coupon_id' => $product['coupon_id'] ?? 0, 'hash' => $hash, 'options_identifier' => $item['options_identifier'], 'options_array' => serialize($item['options']), 'product_options' => $this->serialize_options($item['options'], $item['id'])];
            ...
            $tax_state_id = is_numeric($this->_basket[$tax_on]['state_id']) ? $this->_basket[$tax_on]['state_id'] : get_state_format($this->_basket[$tax_on]['state_id'], 'name', 'id');
            $tax_amount = $record['price'] * $record['quantity'];
            // By this stage product prices will always be excluding tax!
            if (isset($tax_state_id)) {
                Tax::get_instance()->product_tax($tax_amount, (int) $product['tax_type'], false, $tax_state_id);
            } else {
                Tax::get_instance()->product_tax($tax_amount, (int) $product['tax_type'], false);
            }
            if ($record['digital'] && !isset($item['certificate'])) {
                // If digital, create a download code
                $this->_create_download($item['id'], (int) $insert_id);
            }
            return $product;
        }
        return false;
     ...
     */
    private function _order_summary(bool $update = false, $force_order = false)
    {
        ...
        if (($order_notes = $GLOBALS['session']->get(null, 'OrderNotes')) !== false) {
            foreach ($order_notes as $note_source => $note_msg) {
                if ($this->add_note($this->_order_id, $note_source . ':' . $note_msg)) {
        ...
        if ($GLOBALS['db']->select('CubeCart_order_summary', ['cart_order_id'], ['cart_order_id' => $this->_order_id], false, false, false, false)) {
            $this->add_note($this->_order_id, sprintf($GLOBALS['language']->orders['order_updated_via_basket'], $this->_basket['billing_address']['first_name']), false);
            $update = true;
        }
        if ($update) {
            // Update Summary
            $this->update_summary($this->_basket['cart_order_id'], $record);
        } else if ($GLOBALS['db']->insert('CubeCart_order_summary', $record)) {
            $this->set_order_custom_id($this->_basket['cart_order_id']);
            $GLOBALS['user']->add_order($customer_id);
        }
        // Add notes if credit has been used
        if ($record['credit_used'] > 0) {
            $this->add_note($this->_basket['cart_order_id'], sprintf($GLOBALS['language']->orders['credit_note_usage'], Tax::get_instance()->price_format($record['credit_used'])));
        }
    }
    /**
     ...
     * @param string $order_id
     */
    private function _retrieve_order($order_id): bool
    {
     ...
     */
    private function _save_addresses(): void
    {
        if (($addresses = $GLOBALS['user']->get_addresses()) !== false) {
     ...
     * @return bool
     */
    private function _send_coupon($coupon_id, array $data)
    {
        if (!empty($coupon_id)) {
            if (($coupon = $GLOBALS['db']->select('CubeCart_coupons', false, ['coupon_id' => (int) $coupon_id, 'email_sent' => 0])) !== false) {
                $mailer = new Mailer();
                if (isset($data['value'])) {
                    $data['value'] = Tax::get_instance()->price_format($data['value']);
                }
                $data['storeURL'] = $GLOBALS['storeURL'];
                if (($content = $mailer->load_content('cart.gift_certificate', $this->_order_summary['lang'], array_merge($this->_order_summary, $data, $coupon[0]))) !== false) {
                    if (($return = $mailer->send_email($data['email'], $content)) !== false) {
                        $GLOBALS['db']->update('CubeCart_coupons', ['email_sent' => 1], ['coupon_id' => (int) $coupon_id]);
                    } else if (isset($mailer->error_info) && !empty($mailer->error_info)) {
                        trigger_error($mailer->error_info, E_USER_WARNING);
                        $GLOBALS['gui']->set_error($GLOBALS['language']->catalogue['gc_failed'] . ' ' . $GLOBALS['language']->catalogue['gc_specific_error'], true);
                    } else {
                        $GLOBALS['gui']->set_error($GLOBALS['language']->catalogue['gc_failed'], true);
                    }
                    return $return;
                }
            }
     ...
     */
    private function _tidy_orders(): void
    {
        ...
        if (!empty($expire) && is_numeric($expire)) {
            ...
            if (($orders = $GLOBALS['db']->select('CubeCart_order_summary', ['cart_order_id'], ['status' => 1, 'order_date' => '<' . $expire], false, false, false, false)) !== false) {
                foreach ($orders as $order) {
                    // Manage stock
                    $this->_manage_stock(self::ORDER_CANCELLED, $order['cart_order_id']);
                    // Cancel the order
                    $GLOBALS['db']->update('CubeCart_order_summary', ['status' => self::ORDER_CANCELLED], ['cart_order_id' => $order['cart_order_id']]);
                    $log = ['notes' => 'Order cancelled automatically as it has been left in a pending state longer than allowed. See &quot;Time (in seconds) before expiring pending orders&quot; in the &quot;Features&quot; tab of the stores settings to adjust or disable this time limit.', 'order_id' => $order['cart_order_id']];
                    $this->log_transaction($log, true);
                    $this->_add_history($order['cart_order_id'], self::ORDER_CANCELLED, 'E');
                }
            }
        }
    }
     ...
     */
    private function _update_order(): bool
    {
        ...
        if (!isset($this->_basket['order_status']) || $this->_basket['order_status'] < self::ORDER_PROCESS) {
            ...
            if ($order_items) {
                foreach ($order_items as $order_item) {
                    $stored_items[$order_item['hash']] = ['id' => $order_item['id'], 'digital' => $order_item['digital'], 'quantity' => $order_item['quantity']];
            ...
            foreach ($this->_basket['contents'] as $hash => $item) {
                ...
                if (is_array($stored_items[$hash]) && $stored_items[$hash]['quantity'] !== $item['quantity']) {
                    ...
                    $product = $this->_order_add_product($item, $hash);
