...
class Ajax
{
    ...
    public static function load()
    {
        ...
        return match ($function) {
            'rebuildElasticsearch' => self::rebuildElasticsearch(),
            'filesize' => self::filesize((string)$_GET['path'], 0),
            'fmSearch' => self::fmSearch(),
            'doneToggle' => self::doneToggle(),
            'viewEmail' => self::viewEmail((int)$_GET['id'], (string)$_GET['mode']),
            'SMTPTest' => self::SMTPTest(),
            'previewOrderFormat' => self::previewOrderFormat(),
            'template' => self::template($type, $string),
            'subscriber_log' => self::subscriberLog(),
                    ...
                    if (is_array($dirs)) {
                        ...
                        if (isset($dirs[$dir]) && !empty($dirs[$dir])) {
                            foreach ($dirs[$dir] as $folder) {
                                ...
                                $json[] = [
                                    'type' => 'directory',
                                    'path' => urldecode($dir.basename((string) $folder).'/'),
                    ...
                    if (($files = $filemanager->listFiles()) !== false) {
                        ...
                        foreach ($files as $result) {
                            ...
                            $json[] = [
                                ...
                                'file'   => basename((string) $result['filename']),
     ...
     * @return data/false
     */
    public static function previewOrderFormat(): string|false
    {
        if (CC_IN_ADMIN) {
            if ($_GET['oid_mode'] == 't') {
     ...
     * @return data/false
     */
    public static function SMTPTest(): string|false
    {
        if (CC_IN_ADMIN) {
            ...
            if ($GLOBALS['RAW']['POST']['email_method'] == 'sendgrid') {
                ...
                $test_mailer->setFrom($GLOBALS['RAW']['POST']['email_address'], html_entity_decode((string) $GLOBALS['RAW']['POST']['email_name'], ENT_QUOTES));
                $test_mailer->setSubject($subject);
                $test_mailer->addTo($GLOBALS['RAW']['POST']['email_address'], html_entity_decode((string) $GLOBALS['RAW']['POST']['email_name'], ENT_QUOTES));
                ...
                try {
                    ...
                    if (str_starts_with($response->statusCode(), '2')) {
                        $test_success = true;
                    }
                } catch (Exception $e) {
                    $json .= $e->getMessage();
            ...
            if ($GLOBALS['RAW']['POST']['email_method'] !== 'mail') {
                ...
                $test_mailer->FromName  = html_entity_decode((string) $GLOBALS['RAW']['POST']['email_name'], ENT_QUOTES);
     ...
     * @return data/false
     */
    public static function subscriberLog(): string|false
    {
        if (CC_IN_ADMIN) {
