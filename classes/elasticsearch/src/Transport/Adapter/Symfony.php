<?php

/**
 * Elasticsearch PHP Client
 *
 * @link      https://github.com/elastic/elasticsearch-php
 * @copyright Copyright (c) Elasticsearch B.V (https://www.elastic.co)
 * @license   https://opensource.org/licenses/MIT MIT License
 *
 * Licensed to Elasticsearch B.V under one or more agreements.
 * Elasticsearch B.V licenses this file to you under the MIT License.
 * See the LICENSE file in the project root for more information.
 */
declare (strict_types=1);
namespace Elastic\Elasticsearch\Transport\Adapter;

use Elastic\Elasticsearch\Transport\Request_Options;
use Psr\Http\Client\Client_Interface;
use Symfony\Component\Http_Client\Http_Client;
class Symfony implements Adapter_Interface
{
    public function set_config(Client_Interface $client, array $config, array $client_options): Client_Interface
    {
        $symfony_config = [];
        foreach ($config as $key => $value) {
            switch ($key) {
                case Request_Options::SSL_CERT:
                    $symfony_config['local_cert'] = $value;
                    break;
                case Request_Options::SSL_KEY:
                    $symfony_config['local_pk'] = $value;
                    break;
                case Request_Options::SSL_VERIFY:
                    $symfony_config['verify_host'] = $value;
                    $symfony_config['verify_peer'] = $value;
                    break;
                case Request_Options::SSL_CA:
                    $symfony_config['cafile'] = $value;
            }
        }
        $class = $client::class;
        $http_client = Http_Client::create(array_merge($client_options, $symfony_config));
        return new $class($http_client);
    }
}