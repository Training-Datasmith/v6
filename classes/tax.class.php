...
{
    public $_tax_country;
    public $_tax_table_add = [];
    public $_tax_table_inc = [];
    public $_tax_table_applied = [];
    public $_tax_table = [];
    public $_tariff_table = [];
    public $_currency_vars = [];
    ...
    final protected function __construct()
    {
        ...
        if (Config::get_instance()->get('config', 'catalogue_hide_prices') && !User::get_instance()->is() && !CC_IN_ADMIN && !$GLOBALS['session']->has('admin_id', 'admin_data')) {
            Session::get_instance()->set('hide_prices', true);
        } else {
            Session::get_instance()->delete('hide_prices');
        }
        // Switch Currency
        if (isset($_POST['set_currency']) && !empty($_POST['set_currency']) && ($switch = $_POST['set_currency']) || isset($_GET['set_currency']) && !empty($_GET['set_currency']) && $switch = $_GET['set_currency']) {
            if (preg_match('#^[A-Z]{3}$#i', (string) $switch) && $currency = $GLOBALS['db']->select('CubeCart_currency', ['updated'], ['code' => (string) $switch, 'active' => 1])) {
                if (User::get_instance()->is()) {
                    $GLOBALS['db']->update('CubeCart_customer', ['currency' => (string) $switch], ['customer_id' => User::get_instance()->get('customer_id')]);
                }
                $GLOBALS['session']->set('currency', $switch, 'client');
            }
            httpredir(current_page(['set_currency']));
        }
        // Autoload tax tables
        $this->load_currency_vars();
    }
    /**
     ...
     * @return float/false
     */
    public function adjust_tax($total_tax): false|int|float
    {
        if ($this->total_tax() <= 0) {
            return false;
        }
        $reduction = $total_tax / $this->total_tax();
        return $this->_adjust_tax = $reduction;
    }
    /**
     * Display all taxes
     */
    public function display_taxes(): void
    {
        ...
        if (!empty($this->_tax_table_applied)) {
            foreach ($this->_tax_table_applied as $tax_id => $tax_name) {
                if (isset($taxes[$tax_name])) {
                    ...
                    $taxes[$tax_name]['value'] ??= (float) ($this->_tax_table_inc[$tax_id] ?? 0 + $this->_tax_table_add[$tax_id] ?? 0) * $this->_adjust_tax;
                    $taxes[$tax_name]['tax_id'] = $tax_id;
                }
            }
                ...
                if ($taxes['inherited']['value'] > 0) {
                    foreach ($taxes as $tax_name => $tax) {
                        if ($tax_name !== 'inherited') {
                            ...
                            if (round($tax_value, 2) == 0) {
                                continue;
                            }
                            $display_taxes[] = ['name' => $tax_name, 'value' => $this->price_format($tax_value), 'included' => $taxes_included];
                            $basket_taxes[] = ['tax_id' => $tax['tax_id'], 'amount' => $tax_value];
                        }
                    }
                }
            } else {
                foreach ($taxes as $tax_name => $tax) {
                    if (round($tax['value'], 2) == 0) {
                        continue;
                    }
                    $display_taxes[] = ['name' => $tax_name, 'value' => $this->price_format($tax['value']), 'included' => $taxes_included];
                    $basket_taxes[] = ['tax_id' => $tax['tax_id'], 'amount' => $tax['value']];
                }
            }
            $GLOBALS['cart']->set('order_taxes', $basket_taxes);
            $GLOBALS['smarty']->assign('TAXES', $display_taxes);
        }
        $GLOBALS['smarty']->assign('TOTAL_TAX', $this->price_format($this->_total_tax_add + $this->_total_tax_inc));
    }
    /**
     ...
     * @param bool $from
     */
    public function exchange_rate(&$price, $from = false): bool
    {
        if (!empty($from) && $from != $GLOBALS['config']->get('config', 'default_currency')) {
            $currency = $GLOBALS['db']->select('CubeCart_currency', ['value'], ['code' => $from]);
            if ($currency) {
     ...
     */
    public function fetch_tax_amounts(): array
    {
        return ['applied' => $this->_total_tax_add * $this->_adjust_tax, 'included' => $this->_total_tax_inc * $this->_adjust_tax];
    }
    /**
     ...
     * @return array/false
     */
    public function fetch_tax_details($tax_id)
    {
        // Grouped tariffs this will get the right name but the percent may later be wrong
        $grouped = false;
        if (str_contains($tax_id, '|')) {
            $tax_id = explode('|', $tax_id)[0];
            $grouped = true;
        }
        if (str_starts_with((string) $tax_id, 'i')) {
            // import tariff
            $tax_id = (int) substr((string) $tax_id, 1);
            if (($tariff = $GLOBALS['db']->select('CubeCart_tariff', false, ['id' => $tax_id])) !== false) {
                return ['name' => $this->tariff_name($tariff[0]), 'display' => $this->tariff_name($tariff[0]), 'tax_percent' => $grouped ? null : $tariff[0]['percent']];
            }
        }
        if (($rate = $GLOBALS['db']->select('CubeCart_tax_rates', false, ['id' => (int) $tax_id])) !== false) {
            if (($detail = $GLOBALS['db']->select('CubeCart_tax_details', false, ['id' => $rate[0]['details_id']])) !== false) {
                return ['name' => $detail[0]['name'], 'tax_percent' => $rate[0]['tax_percent'], 'display' => $detail[0]['display']];
            }
        }
    }
    /**
     ...
     * @return string
     */
    public function tariff_name(array $tariff)
    {
        if (!empty($tariff['display'])) {
            return $tariff['display'];
            // Work around to preserve duplicate display names
        }
     ...
     * @return float
     */
    public function inclusive_tax_remove(&$price, $tax_type, $type = 'goods'): int|float
    {
        $tax_total = 0;
        if ($tax_type == 999999) {
            $percent = $this->_get_inherited_tax();
            ...
            if (is_array($taxes)) {
                foreach ($taxes as $tax_group) {
                    $tax_table[$tax_group['id']] = ['goods' => (bool) $tax_group['goods'], 'shipping' => (bool) $tax_group['shipping'], 'type' => $tax_group['type_id'], 'name' => !empty($tax_group['display']) ? $tax_group['display'] : $tax_group['name'], 'percent' => $tax_group['tax_percent'], 'county_id' => $tax_group['county_id']];
                }
            }
            if (is_array($tax_table)) {
                foreach ($tax_table as $tax) {
                    if ($tax[$type] && $tax['type'] == $tax_type && in_array($tax['county_id'], [$GLOBALS['config']->get('config', 'store_zone'), 0])) {
    ...
    public function load_currency_vars($code = ''): bool
    {
        ...
        if (($result = $GLOBALS['db']->select('CubeCart_currency', '*', ['code' => $code])) !== false) {
        ...
        if ($code !== $default && ($result = $GLOBALS['db']->select('CubeCart_currency', '*', ['code' => $default])) !== false) {
            $this->_currency_vars = $result[0];
     ...
     */
    public function load_taxes($country_id): void
    {
        if (!empty($country_id)) {
            // Fetch new vars
            ...
            $taxes = $GLOBALS['db']->query($query);
            if (!$taxes) {
                // But ... do we have a Rest of World tax?
            ...
            // Get tariffs
            if ($tariffs = $GLOBALS['db']->select('CubeCart_tariff', false, ['destination' => $GLOBALS['cart']->basket['delivery_address']['country_iso']])) {
                foreach ($tariffs as $tariff) {
                    $this->_tariff_table[$tariff['id']] = ['goods' => 1, 'shipping' => 0, 'tariff' => $tariff['tariff'], 'source' => $tariff['source'], 'destination' => $tariff['destination'], 'percent' => $tariff['percent'], 'display' => $tariff['display']];
                }
            }
        }
    }
     ...
     * @return float
     */
    public function price_convert_fx($price): int|float
    {
     ...
     * @return float
     */
    public function price_correction($price)
    {
        return $price;
    }
    /**
     ...
     * @return string/false
     */
    public function price_format($price, $display_null = true, $default_currency = false, $override_hide = false)
    {
        if ($default_currency) {
            $this->load_currency_vars($GLOBALS['config']->get('config', 'default_currency'));
        }
        $price = $this->_remove_symbol($price);
        if ($display_null && is_numeric($price)) {
            if ($override_hide == false && $GLOBALS['session']->get('hide_prices')) {
                ...
                return $this->price_format_hidden();
            }
            ...
            $string = $this->_currency_vars['symbol_left'] . number_format($price, $decimal_places, empty($this->_currency_vars['symbol_decimal']) ? '.' : $this->_currency_vars['symbol_decimal'], empty($this->_currency_vars['symbol_thousand']) ? ',' : $this->_currency_vars['symbol_thousand']) . $this->_currency_vars['symbol_right'];
     ...
     */
    public function price_format_hidden(): string
    {
     ...
     * @return foat/false
     */
    public function product_tax(&$price, $tax_type, $tax_inclusive = false, $state = 0, $type = 'goods', $sum = true, $is_digital = '0'): array|false
    {
        ...
        if ($is_digital == '1') {
            $check_tariff = false;
            // No tariffs on digital goods
        }
        // Allows a hook to trigger a RETURN of this function by setting a variable
        if (isset($class_tax_producttax_return) && $class_tax_producttax_return === true) {
            return ['tax_id' => $tax_id, 'amount' => $amount, 'amount_raw' => $amount, 'tax_inclusive' => $tax_inclusive, 'tax_name' => $tax_name, 'tax_percent' => $percent];
        }
        if ($price <= 0) {
            return false;
        }
        if ($tax_type == 999999) {
            ...
            return ['tax_id' => $tax_id, 'amount' => $amount, 'amount_raw' => $amount_raw, 'tax_inclusive' => $tax_inclusive, 'tax_name' => 'inherited', 'tax_percent' => $percent];
        }
        if ($check_tariff && is_array($this->_tariff_table) && !empty($this->_tariff_table)) {
            $store_country_iso = get_country_format($GLOBALS['config']->get('config', 'store_country'), 'numcode', 'iso');
            foreach ($this->_tariff_table as $tariff_id => $tariff) {
                if ($tariff['tariff'] == 'M' && $manufacture_country == $tariff['source'] && $GLOBALS['cart']->basket['delivery_address']['country_iso'] == $tariff['destination'] || $tariff['tariff'] == 'D' && $store_country_iso == $tariff['source'] && $GLOBALS['cart']->basket['delivery_address']['country_iso'] == $tariff['destination']) {
                    ...
                    if ($sum) {
                        $this->_tax_table_applied[$tariff_id] = $this->tariff_name($tariff);
        ...
        if (is_array($this->_tax_table) && !empty($this->_tax_table)) {
            ...
            foreach ($this->_tax_table as $tax_id => $tax) {
                if ($tax[$type] && $tax['type'] == $tax_type && in_array($tax['county_id'], [$state, 0])) {
                    $tax_name = $tax['name'];
                    $percent = $tax['percent'];
            ...
            return ['tax_id' => $tax_id, 'amount' => $amount, 'amount_raw' => $amount_raw, 'tax_inclusive' => $tax_inclusive, 'tax_name' => $tax_name, 'tax_percent' => $percent];
        }
        return false;
    }
    /**
     ...
     * @return string/bool
     */
    public function sale_price($normal_price = null, $sale_price = null, $format = true)
    {
        if (Config::get_instance()->has('config', 'catalogue_sale_mode')) {
            switch (Config::get_instance()->get('config', 'catalogue_sale_mode')) {
                case 1:
                    ## Fixed value per item
                    if (!empty($sale_price) && $sale_price > 0 && $sale_price != $normal_price) {
                        return $format ? $this->price_format($sale_price) : $sale_price;
                    }
                    return false;
                case 2:
                    ## Percentage off all stock
                    $value = $normal_price * ((100 - (float) Config::get_instance()->get('config', 'catalogue_sale_percentage')) / 100);
                    if ($value < $normal_price) {
                        return $format ? $this->price_format($value) : $value;
                    }
                // no break
                default:
                    return false;
     ...
     * @return float
     */
    private function _get_inherited_tax(): float|int
    {
     ...
     * @return float
     */
    private function _remove_symbol($price)
    {
