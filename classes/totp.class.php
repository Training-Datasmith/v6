...
class TOTP
{
     ...
     */
    public static function get_code($secret, $offset = 0): string
    {
        ...
        $ofs = ord($hash[19]) & 0xf;
        $code = ((ord($hash[$ofs]) & 0x7f) << 24 | (ord($hash[$ofs + 1]) & 0xff) << 16 | (ord($hash[$ofs + 2]) & 0xff) << 8 | ord($hash[$ofs + 3]) & 0xff) % 1000000;
     ...
     */
    public static function verify_code($secret, $code): bool
    {
        ...
        foreach ([-1, 0, 1] as $offset) {
            if (hash_equals(self::get_code($secret, $offset), $code)) {
                return true;
            }
        }
        return false;
     ...
     */
    public static function otpauth_uri($secret, $label, $issuer): string
    {
