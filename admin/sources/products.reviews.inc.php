...
if (isset($_GET['delete']) && is_numeric($_GET['delete']) && Admin::getInstance()->permissions('reviews', CC_PERM_DELETE)) {
    if ($GLOBALS['db']->delete('CubeCart_reviews', ['id' => (int)$_GET['delete']])) {
...
if (!empty($_POST['delete']['email'])) {
    if ($GLOBALS['db']->delete('CubeCart_reviews', ['email' => $_POST['delete']['email']])) {
    ...
    switch ($_POST['multi-status']) {
        case 'delete':
            if (!empty($_POST['delete']['individual'])) {
                if ($GLOBALS['db']->delete('CubeCart_reviews', ['id' => array_keys($_POST['delete']['individual'])])) {
                    $GLOBALS['main']->successMessage($lang['reviews']['notify_review_deleted']);
                }
            }
            break;
...
if (!empty($_POST['delete']['ip_address'])) {
    if ($GLOBALS['db']->delete('CubeCart_reviews', ['ip_address' => $_POST['delete']['ip_address']])) {
        $GLOBALS['main']->successMessage($lang['reviews']['notify_review_deleted']);
    }
}
...
if (isset($_POST['review']) && is_array($_POST['review']) && Admin::getInstance()->permissions('reviews', CC_PERM_EDIT)) {
    ...
    if (is_numeric($_POST['review']['id'])) {
        $updated = $GLOBALS['db']->update('CubeCart_reviews', $record, ['id' => (int)$_POST['review']['id']]);
...
if (isset($_POST['approve']) && is_array($_POST['approve']) && Admin::getInstance()->permissions('reviews', CC_PERM_EDIT)) {
    ...
    foreach ($_POST['approve'] as $review_id => $status) {
        $GLOBALS['db']->update('CubeCart_reviews', ['approved' => (int)$status], ['id' => (int)$review_id]);
    }
...
if (isset($_POST['filter']) && !empty($_POST['filter'])) {
    ...
    $append  = ['field' => $_POST['field'], 'sort' => $_POST['sort']];
    $anchor  = 'reviews';
    $rem_array  = null;
...
$GLOBALS['gui']->addBreadcrumb($lang['reviews']['title_reviews'], currentPage(['edit', 'field', 'sort', 'product_id', 'approved']));
...
if (isset($failed_review_data)) {
    ...
    for ($i = 1; $i <= 5; $i++) {
        $GLOBALS['smarty']->assign('STAR', ['value' => $i, 'checked' => ($i == ($failed_review_data['rating'] ?? 0)) ? ' checked="checked"' : '']);
    }
    ...
    if ($_GET['edit'] == 0) {
        ...
        $GLOBALS['smarty']->assign('DISPLAY_FORM', true);
    } elseif (($reviews = $GLOBALS['db']->select('CubeCart_reviews', false, ['id' => (int)$_GET['edit']])) !== false) {
        ...
        for ($i = 1; $i <= 5; $i++) {
            $GLOBALS['smarty']->assign('STAR', ['value' => $i, 'checked' => ($i == $review['rating']) ? ' checked="checked"' : '']);
        }
    ...
    $reviews = $GLOBALS['db']->select('CubeCart_reviews', false, $where, [$filter['field'] => $filter['sort']], $per_page, $page);

    if (isset($_GET['product_id']) && is_numeric($_GET['product_id'])) {
        $product = $GLOBALS['db']->select('CubeCart_inventory', ['name'], ['product_id' => (int)$_GET['product_id']]);
    }

    if (!$reviews && isset($product) && $product) {
        $GLOBALS['main']->errorMessage($lang['reviews']['error_reviews_none']);
        httpredir(currentPage(['product_id']), 'search');
    }

    if ($reviews) {
        $GLOBALS['smarty']->assign('PAGINATION', $GLOBALS['db']->pagination(false, $per_page, $page, 9));
        foreach ($reviews as $review) {
            if (($product = $GLOBALS['db']->select('CubeCart_inventory', ['name'], ['product_id' => $review['product_id']])) !== false) {
                $review['product'] = $product[0];
                $review['date']  = formatTime($review['time']);
                $review['delete'] = currentPage(null, ['delete' => (int)$review['id'], 'token' => SESSION_TOKEN]);
                $review['edit']  = currentPage(null, ['edit' => (int)$review['id']]);
                $smarty_data['reviews'][] = $review;
            } else {
                $GLOBALS['db']->delete('CubeCart_reviews', ['product_id' => $review['product_id']]);
            }
        }
    ...
    $fields = [
        ...
        */
        ['value' => 'rating', 'name' => $lang['documents']['rating']],
        ['value' => 'time', 'name' => $lang['common']['date']],
    ];
    $sorts = [
        ['value' => 'DESC', 'name' => $lang['category']['sort_high_low']],
        ['value' => 'ASC', 'name' => $lang['category']['sort_low_high']],
    ];
    $statuses = [
        ['value' => '', 'name' => $lang['common']['all']],
        ['value' => '1', 'name' => $lang['common']['approved']],
        ['value' => '0', 'name' => $lang['common']['unapproved']],
    ];
