...
if (isset($_GET['delete']) && Admin::getInstance()->permissions('settings', CC_PERM_DELETE)) {
    ...
    } elseif ($GLOBALS['language']->deleteLanguage($_GET['delete'])) {
        ...
        foreach ($GLOBALS['RAW']['POST']['string'] as $type => $data) {
            foreach ($data as $name => $value) {
                ...
                if ($existing && $basic == $value) {
                    ...
                    $GLOBALS['db']->update('CubeCart_lang_strings', ['value' => $value], $record);
                    $clear = true;
                } elseif (!$existing && $basic !== $value) {
...
if (isset($_POST['export']) && Admin::getInstance()->permissions('settings', CC_PERM_EDIT)) {
    ...
    httpredir(currentPage(['export'], ['language' => $_GET['export']]));
}
...
if (isset($_GET['install']) && Admin::getInstance()->permissions('settings', CC_PERM_EDIT) && isset($_GET['token']) && $_GET['token'] == SESSION_TOKEN) {
    ...
    httpredir(currentPage(['install', 'token']));
}
...
if (isset($_GET['export'])) {
    ...
    $GLOBALS['gui']->addBreadcrumb($lang_info['title'], currentPage(['export'], ['language' => $_GET['export']]));
    ...
    if (!is_array($foreign_strings)) {
        $foreign_strings = [];
    }
    $lang_info = $GLOBALS['language']->getLanguageInfo($_GET['language']);
    $GLOBALS['gui']->addBreadcrumb($lang_info['title'], currentPage(['type'], ['language' => $_GET['language']]));

    if (($groups = $GLOBALS['language']->getGroups()) !== false) {
        foreach ($groups as $group => $data) {
            $smarty_data['sections'][] = [
                'name'   => $group,
    ...
    if (isset($_REQUEST['type']) && !empty($_REQUEST['type'])) { // The group name or module has been chosen. Load and retrieve the appropriate definitions, strings, and customizations for this language.
        ...
        if (file_exists($_REQUEST['type']) && stripos($_REQUEST['type'], 'modules') !== false) {
            ...
            $strings = isset($foreign_strings[$type]) ? $foreign_strings[$type] : [];
            $custom  = $GLOBALS['language']->getCustom($type, $_GET['language']);
        }

        $GLOBALS['gui']->addBreadcrumb($breadcrumb, currentPage(['key']));
        ...
        if (isset($_GET['key'])) {
            ...
            $GLOBALS['smarty']->assign('BACK', currentPage(['key','type']));
        }
        ...
        foreach ($language_strings_to_search as $keySearchGroup => $arrSearchPhrases) {
            $search_hits[$keySearchGroup] = array_filter($arrSearchPhrases, function ($v) {
                return stripos($v, $GLOBALS['RAW']['POST']['lang_groups_search_phrase']) !== false;
            }); // Filter for simple matches.
            if (empty($search_hits[$keySearchGroup])) {
                unset($search_hits[$keySearchGroup]);
            } // No matches? Do not keep this array element.
        }
        if ($db_strings = $GLOBALS['db']->select('CubeCart_lang_strings', false, ['language' => $_GET['language'], 'value' => '~'.$GLOBALS['RAW']['POST']['lang_groups_search_phrase']])) {
        ...
        if (!empty($search_hits)) {
            $mark = $phrase_group_name = $phrase_group_title = [];
        ...
        $GLOBALS['smarty']->assign('SEARCH_HITS', isset($search_hits) ? $search_hits : []);
    }
    ...
    if (!preg_match('/^(modules)/', $_REQUEST['type'] ?? '')) {
        $GLOBALS['main']->addTabControl($lang['translate']['merge_db_file'], false, currentPage(['language'], ['export' => $_GET['language']]));
    }
} else {
    if (isset($_POST['default_language']) && Admin::getInstance()->permissions('settings', CC_PERM_EDIT)) {
        if (is_array($_POST['domain'])) {
            foreach ($_POST['domain'] as $lang_code => $domain) {
                if (empty($domain)) {
                    $GLOBALS['db']->delete('CubeCart_domains', ['language' => $lang_code]);
                    continue;
                }
                if ($GLOBALS['db']->select('CubeCart_domains', false, ['language' => $lang_code])) {
                    $GLOBALS['db']->update('CubeCart_domains', ['domain' => $domain], ['language' => $lang_code]);
                } else {
                    $GLOBALS['db']->insert('CubeCart_domains', ['domain' => $domain, 'language' => $lang_code]);
                }
            }
        }

        // Build full status array: checkboxes only submit checked values
        $checked = isset($_POST['status']) && is_array($_POST['status']) ? $_POST['status'] : [];
        ...
        foreach ($status_array as $code => $status) {
            if ($status == '1') {
                ...
                $existing = $GLOBALS['db']->select('CubeCart_email_content', ['content_type'], ['language' => $code]);
                $db_types = $existing ? array_column($existing, 'content_type') : [];
    ...
    if (($languageList = $GLOBALS['language']->listLanguages()) !== false) {
        foreach ($languageList as $code => $info) {
            ...
            $info['edit'] = currentPage(null, ['language' => $info['code']]);
            $info['delete'] = currentPage(null, ['delete' => $info['code'], 'token' => SESSION_TOKEN]);
    ...
    if ($json) {
        $api_data = json_decode($json, true);
        if ($api_data && !empty($api_data['languages'])) {
            $installed_codes = is_array($languageList) ? array_keys($languageList) : [];
            $available = [];
            foreach ($api_data['languages'] as $api_lang) {
                if (!in_array($api_lang['code'], $installed_codes)) {
                    $api_lang['install_url'] = currentPage(null, ['install' => $api_lang['code'], 'token' => SESSION_TOKEN]);
                    $available[] = $api_lang;
                }
            }
            if (!empty($available)) {
