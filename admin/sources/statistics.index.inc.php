...
$earliest_order = $GLOBALS['db']->select('CubeCart_order_summary', ['MIN' => 'order_date'], ['status' => $select['status']], ['order_date' => 'ASC']);
...
if (!empty($earliest_order[0]['MIN_order_date'])) {
    ...
    $orders_all = $GLOBALS['db']->select('CubeCart_order_summary', ['total', 'cart_order_id', 'order_date'], ['status' => (int)$select['status']]);
    if ($orders_all) {
    ...
    for ($i = $earliest['year'];$i <= $now['year']; ++$i) {
        ...
        $smarty_data['years'][] = ['value' => $i, 'selected' => $selected];
    }
    ...
    for ($i = 1; $i <= 12; ++$i) {
        ...
        $smarty_data['months'][] = ['value' => $i, 'title' => $month_text, 'selected' => $selected];
    }
    ...
    for ($day = 1; $day <= $monthLength; ++$day) {
        ...
        $smarty_data['days'][] = ['value' => $day, 'selected' => $selected];
    }
    ...
    // Populate dropdowns
    $select_options = ['month' => $monthList];
