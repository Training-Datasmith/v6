...
$admins = $GLOBALS['db']->select('CubeCart_admin_users', ['admin_id', 'username', 'name']);
if ($admins) {
...
if (!isset($_GET[$sort_var]) || !is_array($_GET[$sort_var])) {
    $_GET[$sort_var] = ['time' => 'DESC'];
}
$current_page = currentPage([$sort_var]);
$thead_sort = [
...
$logs_admin = $GLOBALS['db']->select('CubeCart_access_log', false, ['type' => 'B'], $_GET[$sort_var], $per_page, $page_admin);
if ($logs_admin) {
    ...
    foreach ($logs_activity as $log) {
        ...
        if (!empty($log['item_id']) && !empty($log['item_type'])) {
            switch ($log['item_type']) {
                case 'oid':
                    $item = $GLOBALS['db']->select('CubeCart_order_summary', ['cart_order_id'], ['cart_order_id' => $log['item_id']]);
                    ...
                    break;
                case 'prod':
                    $item = $GLOBALS['db']->select('CubeCart_inventory', ['name'], ['product_id' => $log['item_id']]);
                    ...
                    break;
                case 'cat':
                    $item = $GLOBALS['db']->select('CubeCart_category', ['cat_name'], ['cat_id' => $log['item_id']]);
                    ...
                    break;
                case 'doc':
                    $item = $GLOBALS['db']->select('CubeCart_documents', ['doc_name'], ['doc_id' => $log['item_id']]);
...
if (($logs_admin = $GLOBALS['db']->select('CubeCart_access_log', false, ['type' => 'F'], $_GET[$sort_var], $per_page, $page_customer)) !== false) {
