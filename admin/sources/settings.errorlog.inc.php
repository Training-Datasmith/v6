...
if (isset($_POST['adminread'])) {
    ...
    foreach ($_POST['adminread'] as $log_id) {
        $result = $GLOBALS['db']->update('CubeCart_admin_error_log', ['read' => $_POST['admin_error_status']], ['log_id' => $log_id]);
        if (!$changed && $result) {
            $changed = true;
        }
    }
...
if (isset($_POST['systemread'])) {
    ...
    foreach ($_POST['systemread'] as $log_id) {
        $result = $GLOBALS['db']->update('CubeCart_system_error_log', ['read' => $_POST['system_error_status']], ['log_id' => $log_id]);
        if (!$changed && $result) {
            $changed = true;
        }
    }
...
$count_unread = $GLOBALS['db']->count('CubeCart_admin_error_log', 'log_id', ['read' => '0']);
...
$error_log = $GLOBALS['db']->select('CubeCart_admin_error_log', ['message', 'time', 'log_id', 'read'], ['admin_id' => Admin::getInstance()->get('admin_id')], ['time' => 'DESC'], $per_page, $page, false);
$count = 0;
$admin_error_log = false;
if ($error_log && is_array($error_log)) {
    $count = $GLOBALS['db']->getFoundRows();
    foreach ($error_log as $log) {
        $smarty_data['error_log'][] = [
...
if (Admin::getInstance()->superUser()) {
    $count_unread = $GLOBALS['db']->count('CubeCart_system_error_log', 'log_id', ['read' => '0']);
    $GLOBALS['main']->addTabControl($lang['settings']['title_system_error_log'], 'system_error_log', null, null, $count_unread);
    //System errors
    $per_page = 25;
    $page = (isset($_GET['page'])) ? $_GET['page'] : 1;
    $system_error_log = $GLOBALS['db']->select('CubeCart_system_error_log', false, false, ['time' => 'DESC'], $per_page, $page, false);
    $count = $GLOBALS['db']->getFoundRows();
    if (is_array($system_error_log)) {
        foreach ($system_error_log as $log) {
            $smarty_data['system_error_log'][] = [
                'time'   => formatTime($log['time']),
                'message'  => $log['message'],
                'url'  => $log['url'],
