    ...
    if (count($_POST['multi_country']) > 0) {
        foreach ($_POST['multi_country'] as $country => $value) {
            switch ($_POST['multi_country_action']) {
                case 'delete':
                    if ($GLOBALS['db']->delete('CubeCart_geo_country', ['id' => $country])) {
                        $GLOBALS['main']->successMessage($lang['country']['notify_country_delete']);
                    }
                    $GLOBALS['db']->delete('CubeCart_geo_zone', ['country_id' => $country]);
                    break;
                case '0':
                case '1':
                case '2':
                case '3':
                    if ($GLOBALS['db']->update('CubeCart_geo_country', ['status' => (int)$_POST['multi_country_action']], ['id' => $country])) {
...
if (isset($_POST['multi_zone_action']) && !empty($_POST['multi_zone_action']) && strtolower($_POST['multi_zone_action']) == 'delete' && Admin::getInstance()->permissions('settings', CC_PERM_EDIT)) {
    if (is_array($_POST['multi_zone'])) {
        foreach ($_POST['multi_zone'] as $zone => $value) {
            $GLOBALS['db']->delete('CubeCart_geo_zone', ['id' => $zone]);
        }
...
if (isset($_POST['save']) && Admin::getInstance()->permissions('settings', CC_PERM_EDIT)) {
    ...
    if (isset($_POST['country']) && is_array($_POST['country'])) {
        ...
        foreach ($_POST['country'] as $id => $data) {
            $GLOBALS['db']->update('CubeCart_geo_country', $data, ['id' => $id], true);
        }
    ...
    if (isset($_POST['zone']) && is_array($_POST['zone'])) {
        ...
        foreach ($_POST['zone'] as $id => $data) {
            $GLOBALS['db']->update('CubeCart_geo_zone', $data, ['id' => $id], true);
        }
...
if (isset($_GET['delete']) && isset($_GET['id']) && is_numeric($_GET['id']) && Admin::getInstance()->permissions('settings', CC_PERM_DELETE)) {
    switch (strtolower($_GET['delete'])) {
        case 'country':
            $GLOBALS['db']->delete('CubeCart_geo_country', ['id' => $_GET['id']]);
            $GLOBALS['main']->successMessage($lang['country']['notify_country_delete']);
            break;
        case 'zone':
            $GLOBALS['db']->delete('CubeCart_geo_zone', ['id' => $_GET['id']]);
            $GLOBALS['main']->successMessage($lang['country']['notify_zone_delete']);
            break;
    }
    httpredir(currentPage(['delete', 'id']));
}
...
if (($country_count = $GLOBALS['db']->select('CubeCart_geo_country', ['name', 'id'], '', ['name' => 'ASC'])) !== false) {
    ...
    if (($countries = $GLOBALS['db']->select('CubeCart_geo_country', false, false, ['name' => 'ASC'], $per_page, $country_page)) !== false) {
        foreach ($countries as $country) {
            $country['delete']   = currentPage(null, ['delete' => 'country', 'id' => $country['id'], 'token' => SESSION_TOKEN]);
...
if (($zone_count = $GLOBALS['db']->select('CubeCart_geo_zone', ['id'])) !== false) {
    ...
    if (($zones = $GLOBALS['db']->select('CubeCart_geo_zone', false, false, ['country_id' => 'ASC', 'name' => 'ASC'], $per_page, $zone_page)) !== false) {
        foreach ($zones as $zone) {
            $zone['country'] = $country_list[$zone['country_id']];
            $zone['delete']  = currentPage(null, ['delete' => 'zone', 'id' => $zone['id'], 'token' => SESSION_TOKEN]);
