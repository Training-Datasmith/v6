...
if (Admin::getInstance()->superUser()) {
    ...
    $request_log = $GLOBALS['db']->select('CubeCart_request_log', '*', false, ['time' => 'DESC'], $per_page, $page, false);
    ...
    if (is_array($request_log)) {
        foreach ($request_log as $log) {
            ...
            } elseif ($error_code_fd > 0 && in_array($error_code_fd, [4, 5])) {
                $error = true;
            } else {
                $error = false;
            }

            $smarty_data['request_log'][] = [
